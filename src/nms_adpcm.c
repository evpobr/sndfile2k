...
{
    0x4000, 0x4167, 0x42d5, 0x444c, 0x45cb, 0x4752, 0x48e2, 0x4a7a, 0x4c1b, 0x4dc7, 0x4f7a,
    0x5138, 0x52ff, 0x54d1, 0x56ac, 0x5892, 0x5a82, 0x5c7e, 0x5e84, 0x6096, 0x62b4, 0x64dd,
    0x6712, 0x6954, 0x6ba2, 0x6dfe, 0x7066, 0x72dc, 0x7560, 0x77f2, 0x7a93, 0x7d42,
};
...
*/
static inline int nms_adpcm_antilog(int exp)
{
    int ret;
...
*/
static int16_t nms_adpcm_decode_sample(struct nms_adpcm_state *s, uint8_t I)
{
    int sl;
...
static int nms_adpcm_close(SF_PRIVATE *psf)
{
    ...
    */
    if (psf->file.mode == SFM_WRITE)
    {
        ...
        {
            memset(pnms->samples + pnms->sample_curr, 0,
                   (NMS_SAMPLES_PER_BLOCK - pnms->sample_curr) * sizeof(short));
            psf_nms_adpcm_encode_block(psf, pnms);
        }
