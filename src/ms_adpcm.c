...
static int AdaptCoeff1[WAVLIKE_MSADPCM_ADAPT_COEFF_COUNT] = {256, 512, 0,  192,
                                                             240, 460, 392
                                                            };

static int AdaptCoeff2[WAVLIKE_MSADPCM_ADAPT_COEFF_COUNT] = {0, -256, 0,   64,
                                                             0, -208, -232
                                                            };
...
static int msadpcm_read_block(SF_PRIVATE *psf, MSADPCM_PRIVATE *pms, short *ptr,
                              size_t len);
...
int wavlike_msadpcm_init(SF_PRIVATE *psf, int blockalign, int samplesperblock)
{
	...
	pms->block =
	    (unsigned char *)(pms->dummydata + psf->sf.channels * samplesperblock);
	...
	if (psf->file.mode == SFM_READ)
	{
		...
		psf->sf.frames =
		    (psf->datalength / pms->blocksize) * pms->samplesperblock;
...
static inline short msadpcm_get_bpred(SF_PRIVATE *psf, MSADPCM_PRIVATE *pms,
                                      unsigned char value)
{
	if (value >= WAVLIKE_MSADPCM_ADAPT_COEFF_COUNT)
	{
		if (pms->sync_error == 0)
		{
			pms->sync_error = 1;
			psf_log_printf(
			    psf, "MS ADPCM synchronisation error (%u should be < %u).\n",
...
static int msadpcm_decode_block(SF_PRIVATE *psf, MSADPCM_PRIVATE *pms)
{
	...
	*/
	...
	for (k = 2 * pms->channels; k < (pms->samplesperblock * pms->channels); k++)
	{
		...
		chan_idelta[chan] =
		    (AdaptationTable[bytecode] * idelta) >>
		...
		predict =
		    ((pms->samples[k - pms->channels] * AdaptCoeff1[bpred[chan]]) +
		     (pms->samples[k - 2 * pms->channels] *
		      AdaptCoeff2[bpred[chan]])) >>
...
void wavlike_msadpcm_write_adapt_coeffs(SF_PRIVATE *psf)
{
	...
	for (k = 0; k < WAVLIKE_MSADPCM_ADAPT_COEFF_COUNT; k++)
		psf_binheader_writef(psf, "22", BHW2(AdaptCoeff1[k]),
		                     BHW2(AdaptCoeff2[k]));
}
	...
	for (chan = 0; chan < channels; chan++)
	{
		...
		for (bpred = 0; bpred < 7; bpred++)
		{
			...
			for (k = 2; k < 2 + IDELTA_COUNT; k++)
				idelta_sum +=
				    abs(data[k * channels] -
				        ((data[(k - 1) * channels] * AdaptCoeff1[bpred] +
