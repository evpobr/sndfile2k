...
typedef struct sf_private_tag
{
	...
	*/
	...
	sf_count_t
	fileoffset; /* Offset in number of bytes from beginning of file. */

	sf_count_t rsrclength; /* Length of the resource fork (if it exists). */

	sf_count_t
	dataoffset; /* Offset in number of bytes from beginning of file. */
	...
	*/
	...
	size_t (*read_short)(struct sf_private_tag *, short *ptr,
	                     size_t len);
	size_t (*read_int)(struct sf_private_tag *, int *ptr, size_t len);
	size_t (*read_float)(struct sf_private_tag *, float *ptr,
	                     size_t len);
	size_t (*read_double)(struct sf_private_tag *, double *ptr,
	                      size_t len);

	size_t (*write_short)(struct sf_private_tag *, const short *ptr,
	                      size_t len);
	size_t (*write_int)(struct sf_private_tag *, const int *ptr,
	                    size_t len);
	size_t (*write_float)(struct sf_private_tag *, const float *ptr,
	                      size_t len);
	size_t (*write_double)(struct sf_private_tag *, const double *ptr,
	                       size_t len);
	...
	size_t (*command)(struct sf_private_tag *, int command, void *data,
	               size_t datasize);
	...
	*/
	...
	SF_CHUNK_ITERATOR *(*next_chunk_iterator)(struct sf_private_tag *,
	        SF_CHUNK_ITERATOR *iterator);
	int (*get_chunk_size)(struct sf_private_tag *,
	                      const SF_CHUNK_ITERATOR *iterator,
	                      SF_CHUNK_INFO *chunk_info);
	int (*get_chunk_data)(struct sf_private_tag *,
	                      const SF_CHUNK_ITERATOR *iterator,
	                      SF_CHUNK_INFO *chunk_info);
} SF_PRIVATE;
...
size_t psf_fread(void *ptr, size_t bytes, size_t count,
                 SF_PRIVATE *psf);
size_t psf_fwrite(const void *ptr, size_t bytes, size_t count,
                  SF_PRIVATE *psf);
...
int psf_store_read_chunk_str(READ_CHUNKS *pchk, const char *marker,
                             sf_count_t offset, uint32_t len);
...
void psf_f2s_clip_array(const float *src, short *dest, size_t count,
                        int normalize);

void psf_d2s_array(const double *src, short *dest, size_t count, int normalize);
void psf_d2s_clip_array(const double *src, short *dest, size_t count,
                        int normalize);
