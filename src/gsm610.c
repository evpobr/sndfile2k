...
static size_t gsm610_read_block(SF_PRIVATE *psf, GSM610_PRIVATE *pgsm610,
                                short *ptr, size_t len);
static size_t gsm610_write_block(SF_PRIVATE *psf, GSM610_PRIVATE *pgsm610,
...
static int gsm610_wav_decode_block(SF_PRIVATE *psf, GSM610_PRIVATE *pgsm610)
{
	...
	if (gsm_decode(pgsm610->gsm_data,
	               pgsm610->block + (WAVLIKE_GSM610_BLOCKSIZE + 1) / 2,
...
static size_t gsm610_read_s(SF_PRIVATE *psf, short *ptr, size_t len)
{
	...
	while (len > 0)
	{
		readcount = (len > 0x10000000) ? 0x1000000 : len;
                              ...
                              sf_count_t offset)
{
	...
	{
		if (psf->read_current !=
		        newblock * pgsm610->samplesperblock + newsample)
		{
			psf_fseek(psf,
			          psf->dataoffset + newblock * pgsm610->samplesperblock,
			          SEEK_SET);
...
static int gsm610_wav_encode_block(SF_PRIVATE *psf, GSM610_PRIVATE *pgsm610)
{
	...
	/* Encode the samples. */
	gsm_encode(pgsm610->gsm_data, pgsm610->samples, pgsm610->block);
	gsm_encode(pgsm610->gsm_data, pgsm610->samples + WAVLIKE_GSM610_SAMPLES / 2,
