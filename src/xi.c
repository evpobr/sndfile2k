...
static int xi_write_header(SF_PRIVATE *psf, int UNUSED(calc_length))
{
	...
	psf_binheader_writef(psf, "b1", BHWv(pxi->filename),
	                     BHWz(sizeof(pxi->filename)), BHW1(0x1A));

	/* Write software version and two byte XI version. */
	psf_binheader_writef(psf, "eb2", BHWv(pxi->software),
	                     BHWz(sizeof(pxi->software)), BHW2((1 << 8) + 2));
	 ...
	 */
	psf_binheader_writef(psf, "ez2z2", BHWz((size_t)(4 * 3)), BHW2(0x1234),
	                     BHWz(22), BHW2(1));

	pxi->loop_begin = 0;
	pxi->loop_end = 0;

	psf_binheader_writef(psf, "et844", BHW8(psf->sf.frames),
	                     BHW4(pxi->loop_begin), BHW4(pxi->loop_end));

	/* volume, fine tune, flags, pan, note, namelen */
	psf_binheader_writef(psf, "111111", BHW1(128), BHW1(0),
	                     BHW1(pxi->sample_flags), BHW1(128), BHW1(0),
	                     BHW1(strlen(pxi->sample_name)));

	psf_binheader_writef(psf, "b", BHWv(pxi->sample_name),
	                     BHWz(sizeof(pxi->sample_name)));
...
static int xi_read_header(SF_PRIVATE *psf)
{
	...
	{
		psf_binheader_readf(psf, "e444", &(sample_sizes[k]), &loop_begin,
		                    &loop_end);
	...
	{
		psf_log_printf(psf, "*** File seems to be truncated. Should be at "
		               "least %D bytes long.\n",
...
static void dsc2s_array(XI_PRIVATE *pxi, signed char *src, size_t count,
                        short *dest);
static void dsc2i_array(XI_PRIVATE *pxi, signed char *src, size_t count,
                        int *dest);
static void dsc2f_array(XI_PRIVATE *pxi, signed char *src, size_t count,
                        float *dest, float normfact);
static void dsc2d_array(XI_PRIVATE *pxi, signed char *src, size_t count,
                        double *dest, double normfact);

static void dles2s_array(XI_PRIVATE *pxi, short *src, size_t count,
                         short *dest);
static void dles2i_array(XI_PRIVATE *pxi, short *src, size_t count, int *dest);
static void dles2f_array(XI_PRIVATE *pxi, short *src, size_t count, float *dest,
                         float normfact);
static void dles2d_array(XI_PRIVATE *pxi, short *src, size_t count,
                         double *dest,
                         double normfact);
