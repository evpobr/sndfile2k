...
#define MAKE_HASH16(x0, x1, x2, x3, x4, x5, x6, x7, x8, x9, xa, xb, xc, xd, \
                    xe, xf)                                                 \
    ((x0) ^ ((x1) << 1) ^ ((x2) << 2) ^ ((x3) << 3) ^ ((x4) << 4) ^         \
     ((x5) << 5) ^ ((x6) << 6) ^ ((x7) << 7) ^ ((x8) << 8) ^ ((x9) << 9) ^  \
     ((xa) << 10) ^ ((xb) << 11) ^ ((xc) << 12) ^ ((xd) << 13) ^            \
     ((xe) << 14) ^ ((xf) << 15))

#define MAKE_MARKER16(name, x0, x1, x2, x3, x4, x5, x6, x7, x8, x9, xa, xb, \
                      xc, xd, xe, xf)                                       \
...
#define riff_HASH16                                                           \
    MAKE_HASH16('r', 'i', 'f', 'f', 0x2E, 0x91, 0xCF, 0x11, 0xA5, 0xD6, 0x28, \
                0xDB, 0x04, 0xC1, 0x00, 0x00)

#define wave_HASH16                                                           \
    MAKE_HASH16('w', 'a', 'v', 'e', 0xF3, 0xAC, 0xD3, 0x11, 0x8C, 0xD1, 0x00, \
                0xC0, 0x4F, 0x8E, 0xDB, 0x8A)

#define fmt_HASH16                                                            \
    MAKE_HASH16('f', 'm', 't', ' ', 0xF3, 0xAC, 0xD3, 0x11, 0x8C, 0xD1, 0x00, \
                0xC0, 0x4F, 0x8E, 0xDB, 0x8A)

#define fact_HASH16                                                           \
    MAKE_HASH16('f', 'a', 'c', 't', 0xF3, 0xAC, 0xD3, 0x11, 0x8C, 0xD1, 0x00, \
                0xC0, 0x4F, 0x8E, 0xDB, 0x8A)

#define data_HASH16                                                           \
    MAKE_HASH16('d', 'a', 't', 'a', 0xF3, 0xAC, 0xD3, 0x11, 0x8C, 0xD1, 0x00, \
                0xC0, 0x4F, 0x8E, 0xDB, 0x8A)

#define ACID_HASH16                                                         \
    MAKE_HASH16(0x6D, 0x07, 0x1C, 0xEA, 0xA3, 0xEF, 0x78, 0x4C, 0x90, 0x57, \
                0x7F, 0x79, 0xEE, 0x25, 0x2A, 0xAE)

#define levl_HASH16                                                         \
    MAKE_HASH16(0x6c, 0x65, 0x76, 0x6c, 0xf3, 0xac, 0xd3, 0x11, 0xd1, 0x8c, \
                0x00, 0xC0, 0x4F, 0x8E, 0xDB, 0x8A)

#define list_HASH16                                                         \
    MAKE_HASH16(0x6C, 0x69, 0x73, 0x74, 0x2F, 0x91, 0xCF, 0x11, 0xA5, 0xD6, \
                0x28, 0xDB, 0x04, 0xC1, 0x00, 0x00)

#define junk_HASH16                                                         \
    MAKE_HASH16(0x6A, 0x75, 0x6E, 0x6b, 0xF3, 0xAC, 0xD3, 0x11, 0x8C, 0xD1, \
                0x00, 0xC0, 0x4f, 0x8E, 0xDB, 0x8A)

#define bext_HASH16                                                         \
    MAKE_HASH16(0x62, 0x65, 0x78, 0x74, 0xf3, 0xac, 0xd3, 0xaa, 0xd1, 0x8c, \
                0x00, 0xC0, 0x4F, 0x8E, 0xDB, 0x8A)

#define MARKER_HASH16                                                       \
    MAKE_HASH16(0x56, 0x62, 0xf7, 0xab, 0x2d, 0x39, 0xd2, 0x11, 0x86, 0xc7, \
                0x00, 0xc0, 0x4f, 0x8e, 0xdb, 0x8a)

#define SUMLIST_HASH16                                                      \
    MAKE_HASH16(0xBC, 0x94, 0x5F, 0x92, 0x5A, 0x52, 0xD2, 0x11, 0x86, 0xDC, \
                0x00, 0xC0, 0x4F, 0x8E, 0xDB, 0x8A)

MAKE_MARKER16(riff_MARKER16, 'r', 'i', 'f', 'f', 0x2E, 0x91, 0xCF, 0x11, 0xA5,
              0xD6, 0x28, 0xDB, 0x04, 0xC1, 0x00, 0x00);

MAKE_MARKER16(wave_MARKER16, 'w', 'a', 'v', 'e', 0xF3, 0xAC, 0xD3, 0x11, 0x8C,
              0xD1, 0x00, 0xC0, 0x4F, 0x8E, 0xDB, 0x8A);

MAKE_MARKER16(fmt_MARKER16, 'f', 'm', 't', ' ', 0xF3, 0xAC, 0xD3, 0x11, 0x8C,
              0xD1, 0x00, 0xC0, 0x4F, 0x8E, 0xDB, 0x8A);

MAKE_MARKER16(fact_MARKER16, 'f', 'a', 'c', 't', 0xF3, 0xAC, 0xD3, 0x11, 0x8C,
              0xD1, 0x00, 0xC0, 0x4F, 0x8E, 0xDB, 0x8A);

MAKE_MARKER16(data_MARKER16, 'd', 'a', 't', 'a', 0xF3, 0xAC, 0xD3, 0x11, 0x8C,
              0xD1, 0x00, 0xC0, 0x4F, 0x8E, 0xDB, 0x8A);
...
int w64_open(SF_PRIVATE *psf)
{
    ...
    if (psf->file.mode == SFM_WRITE || psf->file.mode == SFM_RDWR)
    {
        ...
        {
            blockalign =
                wavlike_srate2blocksize(psf->sf.samplerate * psf->sf.channels);
            framesperblock = -1;
                           ...
                           int *framesperblock)
{
    ...
    while (!done)
    {
        ...
        case data_HASH16:
            ...
            psf->datalength =
                SF_MIN(chunk_size - 24, psf->filelength - psf->dataoffset);
        ...
        default:
            psf_log_printf(psf, "*** Unknown chunk marker (%X) at position %D "
    ...
    if (psf->blockwidth)
    {
        if (psf->filelength - psf->dataoffset < psf->datalength)
            psf->sf.frames =
                (psf->filelength - psf->dataoffset) / psf->blockwidth;
        else
    ...
    case WAVE_FORMAT_EXTENSIBLE:
        ...
        psf->sf.format =
            SF_FORMAT_W64 | u_bitwidth_to_subformat(psf->bytewidth * 8);
        break;
...
static int w64_write_header(SF_PRIVATE *psf, int calc_length)
{
    ...
    psf_binheader_writef(psf, "eh8hh", BHWh(riff_MARKER16),
                         BHW8(psf->filelength), BHWh(wave_MARKER16),
                         BHWh(fmt_MARKER16));
    ...
    case SF_FORMAT_PCM_32:
        ...
        psf_binheader_writef(psf, "e8224", BHW8(fmt_size),
                             BHW2(WAVE_FORMAT_PCM), BHW2(psf->sf.channels),
        ...
        psf_binheader_writef(psf, "e22",
                             BHW2(psf->bytewidth * psf->sf.channels),
    ...
    case SF_FORMAT_ULAW:
        ...
        /* fmt : format, channels, samplerate */
        psf_binheader_writef(psf, "e8224", BHW8(fmt_size),
                             BHW2(WAVE_FORMAT_MULAW), BHW2(psf->sf.channels),
                             BHW4(psf->sf.samplerate));
        /*  fmt : bytespersec */
        psf_binheader_writef(
            psf, "e4",
        ...
        /*  fmt : blockalign, bitwidth */
        psf_binheader_writef(psf, "e22",
                             BHW2(psf->bytewidth * psf->sf.channels), BHW2(8));
    ...
    case SF_FORMAT_ALAW:
        ...
        /* fmt : format, channels, samplerate */
        psf_binheader_writef(psf, "e8224", BHW8(fmt_size),
                             BHW2(WAVE_FORMAT_ALAW), BHW2(psf->sf.channels),
                             BHW4(psf->sf.samplerate));
        /*  fmt : bytespersec */
        psf_binheader_writef(
            psf, "e4",
    ...
    case SF_FORMAT_IMA_ADPCM:
    {
        ...
        blockalign =
            wavlike_srate2blocksize(psf->sf.samplerate * psf->sf.channels);
        framesperblock =
            2 * (blockalign - 4 * psf->sf.channels) / psf->sf.channels + 1;
        ...
        psf_binheader_writef(psf, "e822", BHW8(fmt_size),
                             BHW2(WAVE_FORMAT_IMA_ADPCM),
                             BHW2(psf->sf.channels));

        /* fmt : samplerate, bytespersec. */
        psf_binheader_writef(psf, "e44", BHW4(psf->sf.samplerate),
                             BHW4(bytespersec));

        /* fmt : blockalign, bitwidth, extrabytes, framesperblock. */
        psf_binheader_writef(psf, "e2222", BHW2(blockalign), BHW2(4), BHW2(2),
                             BHW2(framesperblock));
    };
    ...
    case SF_FORMAT_MS_ADPCM:
    {
        ...
        blockalign =
            wavlike_srate2blocksize(psf->sf.samplerate * psf->sf.channels);
        framesperblock =
            2 + 2 * (blockalign - 7 * psf->sf.channels) / psf->sf.channels;
        bytespersec = (psf->sf.samplerate * blockalign) / framesperblock;
        ...
        /* fmt : blockalign, bitwidth, extrabytes, framesperblock. */
        psf_binheader_writef(psf, "e22222", BHW2(blockalign), BHW2(4),
                             BHW2(extrabytes), BHW2(framesperblock), BHW2(7));
    ...
    case SF_FORMAT_GSM610:
    {
        ...
        bytespersec = (psf->sf.samplerate * WAVLIKE_GSM610_BLOCKSIZE) /
                      WAVLIKE_GSM610_SAMPLES;
        ...
        /* fmt : size, WAV format type, channels. */
        psf_binheader_writef(psf, "e822", BHW8(fmt_size),
                             BHW2(WAVE_FORMAT_GSM610), BHW2(psf->sf.channels));
        ...
        /* fmt : blockalign, bitwidth, extrabytes, framesperblock. */
        psf_binheader_writef(psf, "e2222", BHW2(WAVLIKE_GSM610_BLOCKSIZE),
                             BHW2(0), BHW2(2), BHW2(WAVLIKE_GSM610_SAMPLES));
    };
    ...
    if (add_fact_chunk)
        psf_binheader_writef(psf, "eh88", BHWh(fact_MARKER16),
                             BHW8((sf_count_t)(16 + 8 + 8)),
                             BHW8(psf->sf.frames));

    psf_binheader_writef(psf, "eh8", BHWh(data_MARKER16),
                         BHW8(psf->datalength + 24));
