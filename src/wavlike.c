...
int wavlike_read_fmt_chunk(SF_PRIVATE *psf, int fmtsize)
{
    ...
    case WAVE_FORMAT_MULAW:
        ...
        {
            bytesread +=
                psf_binheader_readf(psf, "2", &(wav_fmt->size20.extrabytes));
    ...
    case WAVE_FORMAT_MS_ADPCM:
        ...
        bytesread += psf_binheader_readf(
            psf, "222", &(wav_fmt->msadpcm.extrabytes),
        ...
        for (k = 0; k < wav_fmt->msadpcm.numcoeffs; k++)
        {
            ...
            bytesread += psf_binheader_readf(
                psf, "22", &(wav_fmt->msadpcm.coeffs[k].coeff1),
    ...
    case WAVE_FORMAT_GSM610:
        ...
        bytesread +=
            psf_binheader_readf(psf, "22", &(wav_fmt->gsm610.extrabytes),
        ...
        bytespersec =
            (wav_fmt->gsm610.samplerate * wav_fmt->gsm610.blockalign) /
            wav_fmt->gsm610.samplesperblock;
    ...
    case WAVE_FORMAT_EXTENSIBLE:
        ...
        else
        {
            ...
            if ((psf->channel_map = calloc(
                     psf->sf.channels, sizeof(psf->channel_map[0]))) == NULL)
            ...
            for (bit = k = 0;
                 bit < ARRAY_LEN(channel_mask_bits) && k < psf->sf.channels;
                 bit++)
            {
                if (wav_fmt->ext.channelmask & (1 << bit))
                {
                    ...
                    {
                        psf_log_printf(psf, "*** More channel map bits than "
                                            "there are channels.\n");
                        break;
                    };
        ...
        bytesread += psf_binheader_readf(
            psf, "422", &(wav_fmt->ext.esf.esf_field1),
    ...
    case WAVE_FORMAT_G721_ADPCM:
        ...
        {
            bytesread +=
                psf_binheader_readf(psf, "22", &(wav_fmt->g72x.extrabytes),
        ...
        {
            bytesread +=
                psf_binheader_readf(psf, "2", &(wav_fmt->g72x.extrabytes));
            psf_log_printf(
                psf, "  Extra Bytes   : %d%s\n", wav_fmt->g72x.extrabytes,
    ...
    {
        psf_log_printf(psf,
                       "*** wavlike_read_fmt_chunk (bytesread > fmtsize)\n");
        return SFE_WAV_BAD_FMT;
    }
    else
...
void wavlike_write_guid(SF_PRIVATE *psf, const EXT_SUBFORMAT *subformat)
{
    psf_binheader_writef(
        psf, "422b", BHW4(subformat->esf_field1), BHW2(subformat->esf_field2),
...
int wavlike_read_bext_chunk(SF_PRIVATE *psf, uint32_t chunksize)
{
    ...
    if (chunksize > WAV_BEXT_MIN_CHUNK_SIZE)
    {
        ...
        bytes += psf_binheader_readf(psf, "b", BHWv(b->coding_history),
                                     BHWz(b->coding_history_size));
    };
...
int wavlike_write_bext_chunk(SF_PRIVATE *psf)
{
    ...
    psf_binheader_writef(
        psf, "m4", BHWm(bext_MARKER),
        BHW4(WAV_BEXT_MIN_CHUNK_SIZE + b->coding_history_size));
    ...
    psf_binheader_writef(psf, "b", BHWv(b->description),
                         BHWz(sizeof(b->description)));
    psf_binheader_writef(psf, "b", BHWv(b->originator),
                         BHWz(sizeof(b->originator)));
    psf_binheader_writef(psf, "b", BHWv(b->originator_reference),
                         BHWz(sizeof(b->originator_reference)));
    psf_binheader_writef(psf, "b", BHWv(b->origination_date),
                         BHWz(sizeof(b->origination_date)));
    psf_binheader_writef(psf, "b", BHWv(b->origination_time),
                         BHWz(sizeof(b->origination_time)));
    psf_binheader_writef(psf, "442", BHW4(b->time_reference_low),
                         BHW4(b->time_reference_high), BHW2(b->version));
    psf_binheader_writef(psf, "b", BHWv(b->umid), BHWz(sizeof(b->umid)));
    psf_binheader_writef(psf, "z", BHWz(190));

    if (b->coding_history_size > 0)
        psf_binheader_writef(psf, "b", BHWv(b->coding_history),
                             BHWz(b->coding_history_size));
...
int wavlike_read_cart_chunk(SF_PRIVATE *psf, uint32_t chunksize)
{
    ...
    bytes += psf_binheader_readf(psf, "b", c->classification,
                                 sizeof(c->classification));
    bytes += psf_binheader_readf(psf, "b", c->out_cue, sizeof(c->out_cue));
    bytes +=
        psf_binheader_readf(psf, "b", c->start_date, sizeof(c->start_date));
    bytes +=
        psf_binheader_readf(psf, "b", c->start_time, sizeof(c->start_time));
    bytes += psf_binheader_readf(psf, "b", c->end_date, sizeof(c->end_date));
    bytes += psf_binheader_readf(psf, "b", c->end_time, sizeof(c->end_time));
    bytes += psf_binheader_readf(psf, "b", c->producer_app_id,
                                 sizeof(c->producer_app_id));
    bytes += psf_binheader_readf(psf, "b", c->producer_app_version,
                                 sizeof(c->producer_app_version));
    bytes += psf_binheader_readf(psf, "b", c->user_def, sizeof(c->user_def));
    bytes += psf_binheader_readf(psf, "e4", &c->level_reference,
                                 sizeof(c->level_reference));

    for (k = 0; k < ARRAY_LEN(c->post_timers); k++)
        bytes += psf_binheader_readf(psf, "b4", &c->post_timers[k].usage,
                                     make_size_t(4), &c->post_timers[k].value);
    ...
    if (chunksize > WAV_CART_MIN_CHUNK_SIZE)
    {
        ...
        bytes += psf_binheader_readf(psf, "b", c->tag_text,
                                     make_size_t(c->tag_text_size));
    };
...
int wavlike_write_cart_chunk(SF_PRIVATE *psf)
{
    ...
    c = psf->cart_16k;
    psf_binheader_writef(psf, "m4", BHWm(cart_MARKER),
                         BHW4(WAV_CART_MIN_CHUNK_SIZE + c->tag_text_size));
    /*
    ...
    psf_binheader_writef(psf, "b", BHWv(c->client_id),
                         BHWz(sizeof(c->client_id)));
    psf_binheader_writef(psf, "b", BHWv(c->category),
                         BHWz(sizeof(c->category)));
    psf_binheader_writef(psf, "b", BHWv(c->classification),
                         BHWz(sizeof(c->classification)));
    psf_binheader_writef(psf, "b", BHWv(c->out_cue), BHWz(sizeof(c->out_cue)));
    psf_binheader_writef(psf, "b", BHWv(c->start_date),
                         BHWz(sizeof(c->start_date)));
    psf_binheader_writef(psf, "b", BHWv(c->start_time),
                         BHWz(sizeof(c->start_time)));
    psf_binheader_writef(psf, "b", BHWv(c->end_date),
                         BHWz(sizeof(c->end_date)));
    psf_binheader_writef(psf, "b", BHWv(c->end_time),
                         BHWz(sizeof(c->end_time)));
    psf_binheader_writef(psf, "b", BHWv(c->producer_app_id),
                         BHWz(sizeof(c->producer_app_id)));
    psf_binheader_writef(psf, "b", BHWv(c->producer_app_version),
                         BHWz(sizeof(c->producer_app_version)));
    psf_binheader_writef(psf, "b", BHWv(c->user_def),
                         BHWz(sizeof(c->user_def)));
    psf_binheader_writef(psf, "e4", BHW4(c->level_reference));

    for (k = 0; k < ARRAY_LEN(c->post_timers); k++)
        psf_binheader_writef(psf, "b4", BHWv(c->post_timers[k].usage), BHWz(4),
                             BHW4(c->post_timers[k].value));

    psf_binheader_writef(
        psf, "z",
        BHWz(sizeof(
            c->reserved))); // just write zeros, we don't have any other use for it
    psf_binheader_writef(psf, "b", BHWv(c->url), BHWz(sizeof(c->url)));

    if (c->tag_text_size > 0)
        psf_binheader_writef(psf, "b", BHWv(c->tag_text),
                             BHWz(c->tag_text_size));
...
int wavlike_subchunk_parse(SF_PRIVATE *psf, int chunk, uint32_t chunk_length)
{
    ...
    while (bytesread < chunk_length)
    {
        ...
        case 0:
			...
			*/
            ...
            if (bytesread < chunk_length)
                bytesread +=
                    psf_binheader_readf(psf, "j", chunk_length - bytesread + 4);
        ...
        case ITRK_MARKER:
            ...
            if (chunk_size >= SIGNED_SIZEOF(buffer) ||
        ...
        case labl_MARKER:
        {
            ...
            chunk_size -= 4;
            chunk_size += (chunk_size & 1);
            if (chunk_size < 1 || chunk_size >= SIGNED_SIZEOF(buffer) ||
                chunk_size >= chunk_length)
            {
                psf_log_printf(psf, "  *** %M : %u (too big)\n", chunk,
                               chunk_size);
...
void wavlike_write_strings(SF_PRIVATE *psf, int location)
{
    ...
    psf_binheader_writef(psf, "m4m", BHWm(LIST_MARKER), BHW4(0xBADBAD),
                         BHWm(INFO_MARKER));
    ...
    for (k = 0; k < SF_MAX_STRINGS; k++)
    {
        ...
        case SF_STR_SOFTWARE:
            psf_binheader_writef(
                psf, "ms", BHWm(ISFT_MARKER),
                BHWs(psf->strings.storage + psf->strings.data[k].offset));
            break;

        case SF_STR_TITLE:
            psf_binheader_writef(
                psf, "ms", BHWm(INAM_MARKER),
                BHWs(psf->strings.storage + psf->strings.data[k].offset));
            break;

        case SF_STR_COPYRIGHT:
            psf_binheader_writef(
                psf, "ms", BHWm(ICOP_MARKER),
                BHWs(psf->strings.storage + psf->strings.data[k].offset));
            break;

        case SF_STR_ARTIST:
            psf_binheader_writef(
                psf, "ms", BHWm(IART_MARKER),
                BHWs(psf->strings.storage + psf->strings.data[k].offset));
            break;

        case SF_STR_COMMENT:
            psf_binheader_writef(
                psf, "ms", BHWm(ICMT_MARKER),
                BHWs(psf->strings.storage + psf->strings.data[k].offset));
            break;

        case SF_STR_DATE:
            psf_binheader_writef(
                psf, "ms", BHWm(ICRD_MARKER),
                BHWs(psf->strings.storage + psf->strings.data[k].offset));
            break;

        case SF_STR_GENRE:
            psf_binheader_writef(
                psf, "ms", BHWm(IGNR_MARKER),
                BHWs(psf->strings.storage + psf->strings.data[k].offset));
            break;

        case SF_STR_ALBUM:
            psf_binheader_writef(
                psf, "ms", BHWm(IPRD_MARKER),
                BHWs(psf->strings.storage + psf->strings.data[k].offset));
            break;

        case SF_STR_TRACKNUMBER:
            psf_binheader_writef(
                psf, "ms", BHWm(ITRK_MARKER),
                BHWs(psf->strings.storage + psf->strings.data[k].offset));
            break;
    ...
    psf_binheader_writef(psf, "4",
                         BHW4(saved_head_index - prev_head_index - 4));
...
int wavlike_read_peak_chunk(SF_PRIVATE *psf, size_t chunk_size)
{
    ...
    if (chunk_size != WAVLIKE_PEAK_CHUNK_SIZE(psf->sf.channels))
    {
        ...
        psf_log_printf(psf, "*** File PEAK chunk size doesn't fit with number "
                            "of channels (%d).\n",
                       psf->sf.channels);
    ...
    for (uk = 0; uk < (uint32_t)psf->sf.channels; uk++)
    {
        ...
        snprintf(buffer, sizeof(buffer), "    %2d   %-12" PRId64 "   %g\n", uk,
                 psf->peak_info->peaks[uk].position,
...
void wavlike_write_peak_chunk(SF_PRIVATE *psf)
{
    ...
    psf_binheader_writef(psf, "m4", BHWm(PEAK_MARKER),
                         BHW4(WAVLIKE_PEAK_CHUNK_SIZE(psf->sf.channels)));
    psf_binheader_writef(psf, "44", BHW4(1), BHW4(time(NULL)));
    for (k = 0; k < psf->sf.channels; k++)
        psf_binheader_writef(psf, "ft8", BHWf(psf->peak_info->peaks[k].value),
    ...
    while (bytesread < length)
    {
        ...
        case ever_MARKER:
            ...
            vmajor = 10 * (((dword >> 24) & 0xff) - '0') +
                     (((dword >> 16) & 0xff) - '0');
            vminor =
                10 * (((dword >> 8) & 0xff) - '0') + ((dword & 0xff) - '0');
        ...
        case eucm_MARKER: /* user comment: 4-byte size follows, then possibly unicode data */
            ...
            if (marker == emdl_MARKER &&
                dword == strlen(buf) /* should be >= strlen+1*/)
            {
                psf_log_printf(psf, "    *** field size too small for string "
                                    "(sinking 2 bytes)\n");
...
void wavlike_write_custom_chunks(SF_PRIVATE *psf)
{
    ...
    for (k = 0; k < psf->wchunks.used; k++)
        psf_binheader_writef(psf, "m4b", BHWm(psf->wchunks.chunks[k].mark32),
                             BHW4(psf->wchunks.chunks[k].len),
                             BHWv(psf->wchunks.chunks[k].data),
