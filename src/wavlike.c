...
int wavlike_read_fmt_chunk(SF_PRIVATE *psf, int fmtsize)
{
    ...
    case WAVE_FORMAT_MS_ADPCM:
        ...
        for (k = 0; k < wav_fmt->msadpcm.numcoeffs; k++)
        {
            ...
            bytesread += psf_binheader_readf(psf, "22", &(wav_fmt->msadpcm.coeffs[k].coeff1),
    ...
    case WAVE_FORMAT_EXTENSIBLE:
        ...
        bytesread +=
            psf_binheader_readf(psf, "422", &(wav_fmt->ext.esf.esf_field1),
    ...
    if (bytesread > fmtsize)
    {
		...
		psf_binheader_seekf(psf, fmtsize - bytesread, SF_SEEK_CUR);
...
{
    psf_binheader_writef(psf, "422b", BHW4(subformat->esf_field1), BHW2(subformat->esf_field2),
...
int wavlike_read_bext_chunk(SF_PRIVATE *psf, uint32_t chunksize)
{
    ...
    if (chunksize < WAV_BEXT_MIN_CHUNK_SIZE)
    {
		...
		psf_binheader_seekf(psf, chunksize, SF_SEEK_CUR);
        return 0;
    };
    ...
    bytes += psf_binheader_readf(psf, "442", &b->time_reference_low, &b->time_reference_high,
                                 &b->version);
	bytes += psf_binheader_readf(psf, "b", &b->umid, sizeof(b->umid));
	psf_binheader_seekf(psf, 190, SF_SEEK_CUR);
	bytes += 190;
    ...
    if (chunksize > WAV_BEXT_MIN_CHUNK_SIZE)
    {
        ...
        bytes +=
            psf_binheader_readf(psf, "b", BHWv(b->coding_history), BHWz(b->coding_history_size));
    };

    if (bytes < chunksize)
		psf_binheader_seekf(psf, chunksize - bytes, SF_SEEK_CUR);
...
int wavlike_write_bext_chunk(SF_PRIVATE *psf)
{
    ...
    psf_binheader_writef(psf, "m4", BHWm(bext_MARKER),
    ...
    psf_binheader_writef(psf, "442", BHW4(b->time_reference_low), BHW4(b->time_reference_high),
                         BHW2(b->version));
...
int wavlike_read_cart_chunk(SF_PRIVATE *psf, uint32_t chunksize)
{
    ...
    bytes +=
        psf_binheader_readf(psf, "b", c->producer_app_version, sizeof(c->producer_app_version));
    ...
    for (k = 0; k < ARRAY_LEN(c->post_timers); k++)
        bytes += psf_binheader_readf(psf, "b4", &c->post_timers[k].usage, make_size_t(4),
...
int wavlike_write_cart_chunk(SF_PRIVATE *psf)
{
    ...
    psf_binheader_writef(
        psf, "z",
        BHWz(sizeof(c->reserved))); // just write zeros, we don't have any other use for it
...
int wavlike_subchunk_parse(SF_PRIVATE *psf, int chunk, uint32_t chunk_length)
{
    ...
    if (chunk_length <= 8)
    {
        ...
        psf_binheader_readf(psf, "m", &chunk);
		psf_binheader_seekf(psf, chunk_length - 4, SF_SEEK_CUR);
    ...
    while (bytesread < chunk_length)
    {
        ...
        case ITRK_MARKER:
            ...
            if (chunk_size >= (uint32_t)sizeof(buffer) || chunk_size >= chunk_length)
            {
        ...
        case labl_MARKER:
        {
            ...
            chunk_size -= 4;
            chunk_size += (chunk_size & 1);
            if (chunk_size < 1 || chunk_size >= (uint32_t)sizeof(buffer) || chunk_size >= chunk_length)
            {
                psf_log_printf(psf, "  *** %M : %u (too big)\n", chunk, chunk_size);
                goto cleanup_subchunk_parse;
            };
...
void wavlike_write_strings(SF_PRIVATE *psf, int location)
{
    ...
    for (k = 0; k < SF_MAX_STRINGS; k++)
    {
        ...
        case SF_STR_SOFTWARE:
            psf_binheader_writef(psf, "ms", BHWm(ISFT_MARKER),
                                 BHWs(psf->strings.storage + psf->strings.data[k].offset));
            break;

        case SF_STR_TITLE:
            psf_binheader_writef(psf, "ms", BHWm(INAM_MARKER),
                                 BHWs(psf->strings.storage + psf->strings.data[k].offset));
            break;

        case SF_STR_COPYRIGHT:
            psf_binheader_writef(psf, "ms", BHWm(ICOP_MARKER),
                                 BHWs(psf->strings.storage + psf->strings.data[k].offset));
            break;

        case SF_STR_ARTIST:
            psf_binheader_writef(psf, "ms", BHWm(IART_MARKER),
                                 BHWs(psf->strings.storage + psf->strings.data[k].offset));
            break;

        case SF_STR_COMMENT:
            psf_binheader_writef(psf, "ms", BHWm(ICMT_MARKER),
                                 BHWs(psf->strings.storage + psf->strings.data[k].offset));
            break;

        case SF_STR_DATE:
            psf_binheader_writef(psf, "ms", BHWm(ICRD_MARKER),
                                 BHWs(psf->strings.storage + psf->strings.data[k].offset));
            break;

        case SF_STR_GENRE:
            psf_binheader_writef(psf, "ms", BHWm(IGNR_MARKER),
                                 BHWs(psf->strings.storage + psf->strings.data[k].offset));
            break;

        case SF_STR_ALBUM:
            psf_binheader_writef(psf, "ms", BHWm(IPRD_MARKER),
                                 BHWs(psf->strings.storage + psf->strings.data[k].offset));
            break;

        case SF_STR_TRACKNUMBER:
            psf_binheader_writef(psf, "ms", BHWm(ITRK_MARKER),
                                 BHWs(psf->strings.storage + psf->strings.data[k].offset));
            break;
...
static int exif_fill_and_sink(SF_PRIVATE *psf, char *buf, size_t bufsz, size_t toread)
{
	...
	{
		psf_binheader_seekf(psf, toread - bufsz, SF_SEEK_CUR);
    ...
    while (bytesread < length)
    {
        ...
        case olym_MARKER:
            ...
            if (dword > length || bytesread + dword > length)
                break;
            dword += (dword & 1);
			psf_binheader_seekf(psf, dword, SF_SEEK_CUR);
            bytesread += dword;
            break;
