...
static int AdaptCoeff1[WAVLIKE_MSADPCM_ADAPT_COEFF_COUNT] = {256, 512, 0, 192, 240, 460, 392};

static int AdaptCoeff2[WAVLIKE_MSADPCM_ADAPT_COEFF_COUNT] = {0, -256, 0, 64, 0, -208, -232};
...
int wavlike_msadpcm_init(SndFile *psf, int blockalign, int samplesperblock)
{
    ...
    if (!(psf->m_codec_data = calloc(1, pmssize)))
        return SFE_MALLOC_FAILED;
    pms = (MSADPCM_PRIVATE *)psf->m_codec_data;
    ...
    {
        pms->dataremaining = psf->m_datalength;

        if (psf->m_datalength % pms->blocksize)
            pms->blocks = psf->m_datalength / pms->blocksize + 1;
        else
            pms->blocks = psf->m_datalength / pms->blocksize;
        ...
        psf->sf.frames = (psf->m_datalength / pms->blocksize) * pms->samplesperblock;
...
static int msadpcm_decode_block(SndFile *psf, MSADPCM_PRIVATE *pms)
{
    ...
    */
    ...
    for (k = 2 * pms->channels; k < (pms->samplesperblock * pms->channels); k++)
    {
        ...
        chan_idelta[chan] = (AdaptationTable[bytecode] * idelta) >>
        ...
        predict = ((pms->samples[k - pms->channels] * AdaptCoeff1[bpred[chan]]) +
                   (pms->samples[k - 2 * pms->channels] * AdaptCoeff2[bpred[chan]])) >>
...
static sf_count_t msadpcm_seek(SndFile *psf, int mode, sf_count_t offset)
{
    ...
    if (mode == SFM_READ)
    {
        psf->fseek(psf->m_dataoffset + newblock * pms->blocksize, SEEK_SET);
    ...
    if (pms->channels == 1)
    {
        ...
        for (k = 2; k < pms->samplesperblock; k++)
        {
            predict = (pms->samples[k - 1] * AdaptCoeff1[bpred[0]] +
                       pms->samples[k - 2] * AdaptCoeff2[bpred[0]]) >>
                      8;
    ...
    else
    {
        ...
        for (k = 4; k < 2 * pms->samplesperblock; k++)
        {
            ...
            predict = (pms->samples[k - 2] * AdaptCoeff1[bpred[chan]] +
                       pms->samples[k - 4] * AdaptCoeff2[bpred[chan]]) >>
                      8;
...
static void choose_predictor(unsigned int channels, short *data, int *block_pred, int *idelta)
{
    ...
    for (chan = 0; chan < channels; chan++)
    {
        ...
        for (bpred = 0; bpred < 7; bpred++)
        {
            ...
            for (k = 2; k < 2 + IDELTA_COUNT; k++)
                idelta_sum +=
                    abs(data[k * channels] - ((data[(k - 1) * channels] * AdaptCoeff1[bpred] +
                                               data[(k - 2) * channels] * AdaptCoeff2[bpred]) >>
                                              8));
