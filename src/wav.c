...
static SF_CHUNK_ITERATOR *wav_next_chunk_iterator(SF_PRIVATE *psf,
                                                  SF_CHUNK_ITERATOR *iterator);
static int wav_get_chunk_size(SF_PRIVATE *psf,
                              const SF_CHUNK_ITERATOR *iterator,
                              SF_CHUNK_INFO *chunk_info);
static int wav_get_chunk_data(SF_PRIVATE *psf,
                              const SF_CHUNK_ITERATOR *iterator,
                              SF_CHUNK_INFO *chunk_info);
                           ...
                           int *framesperblock)
{
	 ...
	 */
    psf->rwf_endian =
        (marker == RIFF_MARKER) ? SF_ENDIAN_LITTLE : SF_ENDIAN_BIG;
    ...
    while (!done)
    {
        ...
        case fact_MARKER:
            ...
            if (chunk_size > SIGNED_SIZEOF(fact_chunk))
                psf_binheader_readf(
                    psf, "j", (int)(chunk_size - SIGNED_SIZEOF(fact_chunk)));
        ...
        case cue_MARKER:
...

            {
                ...
                while (cue_count)
                {
                    if ((thisread = psf_binheader_readf(
                             psf, "e44m444", &id, &position, &chunk_id,
                    ...
                    psf_log_printf(psf, "   Cue ID : %2d"
                                        ...
                                        "  Offset : %5d\n",
                                   id, position, chunk_id, chunk_start,
                                   block_start, offset);
    ...
    case WAVE_FORMAT_PCM:
        psf->sf.format =
            SF_FORMAT_WAV | u_bitwidth_to_subformat(psf->bytewidth * 8);
        break;
...
static int wav_write_fmt_chunk(SF_PRIVATE *psf)
{
    ...
    case SF_FORMAT_PCM_32:
        ...
        /*  fmt : bytespersec */
        psf_binheader_writef(psf, "4", BHW4(psf->sf.samplerate *
    ...
    case SF_FORMAT_DOUBLE:
        ...
        /* fmt : format, channels, samplerate */
        psf_binheader_writef(psf, "4224", BHW4(fmt_size),
    ...
    case SF_FORMAT_IMA_ADPCM:
    {
        ...
        psf_binheader_writef(psf, "42244", BHW4(fmt_size),
                             BHW2(WAVE_FORMAT_IMA_ADPCM),
                             BHW2(psf->sf.channels), BHW4(psf->sf.samplerate),
                             BHW4(bytespersec));

        /* fmt : blockalign, bitwidth, extrabytes, framesperblock. */
        psf_binheader_writef(psf, "2222", BHW2(blockalign), BHW2(4), BHW2(2),
                             BHW2(framesperblock));
    };
    ...
    case SF_FORMAT_MS_ADPCM:
    {
        ...
        psf_binheader_writef(psf, "422", BHW4(fmt_size),
                             BHW2(WAVE_FORMAT_MS_ADPCM),
                             BHW2(psf->sf.channels));

        /* fmt : samplerate, bytespersec. */
        psf_binheader_writef(psf, "44", BHW4(psf->sf.samplerate),
                             BHW4(bytespersec));

        /* fmt : blockalign, bitwidth, extrabytes, framesperblock. */
        psf_binheader_writef(psf, "22222", BHW2(blockalign), BHW2(4),
    ...
    case SF_FORMAT_GSM610:
    {
        ...
        /* fmt : blockalign, bitwidth, extrabytes, framesperblock. */
        psf_binheader_writef(psf, "2222", BHW2(blockalign), BHW2(0), BHW2(2),
                             BHW2(framesperblock));
    };
    ...
    if (add_fact_chunk)
        psf_binheader_writef(psf, "tm48", BHWm(fact_MARKER), BHW4(4),
                             BHW8(psf->sf.frames));
        ...
        else
        {
            ...
            case 6: /* 5.1 */
                psf_binheader_writef(psf, "4",
                                     BHW4(0x1 | 0x2 | 0x4 | 0x8 | 0x10 | 0x20));
                break;

            case 8: /* 7.1 */
                psf_binheader_writef(psf, "4", BHW4(0x1 | 0x2 | 0x4 | 0x8 |
                                                    0x10 | 0x20 | 0x40 | 0x80));
                break;
...
static int wav_write_header(SF_PRIVATE *psf, int calc_length)
{
    ...
    if (calc_length)
    {
        ...
        if (psf->dataend)
            ...
            psf->datalength =
                psf->sf.frames * psf->bytewidth * psf->sf.channels;
    };
    ...
    if (psf->endian == SF_ENDIAN_LITTLE)
        psf_binheader_writef(
            psf, "etm8", BHWm(RIFF_MARKER),
            BHW8((psf->filelength < 8) ? 8 : psf->filelength - 8));
    else
        psf_binheader_writef(
            psf, "Etm8", BHWm(RIFX_MARKER),
            BHW8((psf->filelength < 8) ? 8 : psf->filelength - 8));
    ...
    if (psf->cues != NULL)
    {
        ...
        for (k = 0; k < psf->cues->cue_count; k++)
            psf_binheader_writef(psf, "e44m444",
                                 BHW4(psf->cues->cue_points[k].indx),
    ...
    if (psf->instrument != NULL)
    {
        ...
        tmp = (int)(1.0e9 /
                    psf->sf.samplerate); /* Sample period in nano seconds */
        psf_binheader_writef(psf, "44", BHW4(tmp),
                             BHW4(psf->instrument->basenote));
        ...
        psf_binheader_writef(psf, "44", BHW4(psf->instrument->loop_count),
                             BHW4(0));
        ...
        for (tmp = 0; tmp < psf->instrument->loop_count; tmp++)
        {
            ...
            type = (type == SF_LOOP_FORWARD
                        ? 0
                        : type == SF_LOOP_BACKWARD
                              ? 2
                              : type == SF_LOOP_ALTERNATING ? 1 : 32);

            psf_binheader_writef(psf, "44", BHW4(tmp), BHW4(type));
            psf_binheader_writef(psf, "44",
                                 BHW4(psf->instrument->loops[tmp].start),
                       ...
                       int datasize)
{
    ...
    case SFC_SET_CHANNEL_MAP_INFO:
        wpriv->wavex_channelmask =
            wavlike_gen_channel_mask(psf->channel_map, psf->sf.channels);
...
static int wav_read_smpl_chunk(SF_PRIVATE *psf, uint32_t chunklen)
{
    ...
    snprintf(buffer, sizeof(buffer), "%02d:%02d:%02d %02d",
             (dword >> 24) & 0x7F, (dword >> 16) & 0x7F, (dword >> 8) & 0x7F,
             dword & 0x7F);
