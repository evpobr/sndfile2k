...
static size_t dwvw_write_s(SF_PRIVATE *psf, const short *ptr,
                           size_t len);
...
static size_t dwvw_decode_data(SF_PRIVATE *psf, DWVW_PRIVATE *pdwvw, int *ptr,
                               size_t len);
static int dwvw_decode_load_bits(SF_PRIVATE *psf, DWVW_PRIVATE *pdwvw,
                                 int bit_count);

static size_t dwvw_encode_data(SF_PRIVATE *psf, DWVW_PRIVATE *pdwvw,
                               const int *ptr, size_t len);
                                 ...
                                 int bit_count)
{
	...
	*/
	...
	while (pdwvw->bit_count < bit_count)
	{
		...
		{
			pdwvw->b.end =
			    psf_fread(pdwvw->b.buffer, 1, sizeof(pdwvw->b.buffer), psf);
			pdwvw->b.index = 0;
		};
                                   ...
                                   int data, int new_bits)
{
	...
	pdwvw->bits = arith_shift_left(pdwvw->bits, new_bits) |
	              (data & (arith_shift_left(1, new_bits) - 1));
                               ...
                               const int *ptr, size_t len)
{
	...
	for (count = 0; count < len; count++)
	{
		...
		delta_width_modifier =
		    (delta_width - pdwvw->last_delta_width) % pdwvw->bit_width;
