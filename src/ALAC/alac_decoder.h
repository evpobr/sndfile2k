...
typedef enum { false = 0, ALAC_TRUE = 1 } bool;
...
int32_t alac_init(alac_decoder *p, void *inMagicCookie,
                  uint32_t inMagicCookieSize);
int32_t alac_decode(alac_decoder *, struct BitBuffer *bits,
                    uint8_t *sampleBuffer, uint32_t numSamples,
                    uint32_t numChannels, uint32_t *outNumSamples);
