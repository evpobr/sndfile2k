...
static sf_count_t paf24_write_s(SF_PRIVATE *psf, const short *ptr,
                                sf_count_t len);
static sf_count_t paf24_write_i(SF_PRIVATE *psf, const int *ptr,
                                sf_count_t len);
...
static int paf_write_header(SF_PRIVATE *psf, int UNUSED(calc_length))
{
    ...
    if (psf->endian == SF_ENDIAN_BIG)
    {
        ...
        psf_binheader_writef(psf, "E444", BHW4(paf_format),
                             BHW4(psf->sf.channels), BHW4(0));
    }
    ...
    psf_binheader_writef(psf, "z",
                         BHWz((size_t)(psf->dataoffset - psf->header.indx)));
...
static sf_count_t paf24_seek(SF_PRIVATE *psf, int mode, sf_count_t offset)
{
    ...
    case SFM_READ:
        ...
        psf_fseek(psf, psf->dataoffset + newblock * ppaf24->blocksize,
                  SEEK_SET);
...
static int paf24_read_block(SF_PRIVATE *psf, PAF24_PRIVATE *ppaf24)
{
    ...
    for (k = 0; k < PAF24_SAMPLES_PER_BLOCK * ppaf24->channels; k++)
    {
        ...
        cptr = ((unsigned char *)ppaf24->block) + PAF24_BLOCK_SIZE * channel +
               3 * (k / ppaf24->channels);
        ppaf24->samples[k] =
            (cptr[0] << 8) | (cptr[1] << 16) | (((unsigned)cptr[2]) << 24);
    };
...
static int paf24_read(SF_PRIVATE *psf, PAF24_PRIVATE *ppaf24, int *ptr, int len)
{
    ...
    while (total < len)
    {
        if (ppaf24->read_block * PAF24_SAMPLES_PER_BLOCK >=
            ppaf24->sample_count)
        {
        ...
        count =
            (PAF24_SAMPLES_PER_BLOCK - ppaf24->read_count) * ppaf24->channels;
        count = (len - total > count) ? count : len - total;

        memcpy(&(ptr[total]),
               &(ppaf24->samples[ppaf24->read_count * ppaf24->channels]),
               count * sizeof(int));
        total += count;
...
static sf_count_t paf24_read_d(SF_PRIVATE *psf, double *ptr, sf_count_t len)
{
    ...
    normfact =
        (psf->norm_double == SF_TRUE) ? (1.0 / 0x80000000) : (1.0 / 0x100);
...
static int paf24_write_block(SF_PRIVATE *psf, PAF24_PRIVATE *ppaf24)
{
    ...
    if (CPU_IS_LITTLE_ENDIAN)
    {
        ...
        {
            channel = k % ppaf24->channels;
            cptr = ((unsigned char *)ppaf24->block) +
                   PAF24_BLOCK_SIZE * channel + 3 * (k / ppaf24->channels);
    ...
    if (ppaf24->sample_count <
        ppaf24->write_block * PAF24_SAMPLES_PER_BLOCK + ppaf24->write_count)
        ppaf24->sample_count =
            ppaf24->write_block * PAF24_SAMPLES_PER_BLOCK + ppaf24->write_count;
                       ...
                       int len)
{
    ...
    {
        count =
            (PAF24_SAMPLES_PER_BLOCK - ppaf24->write_count) * ppaf24->channels;
                                ...
                                sf_count_t len)
{
    ...
    normfact =
        (psf->norm_double == SF_TRUE) ? (1.0 * 0x7FFFFFFF) : (1.0 / 0x100);
