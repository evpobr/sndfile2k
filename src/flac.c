...
static size_t flac_read_flac2s(SF_PRIVATE *psf, short *ptr, size_t len);
static size_t flac_read_flac2i(SF_PRIVATE *psf, int *ptr, size_t len);
static size_t flac_read_flac2f(SF_PRIVATE *psf, float *ptr, size_t len);
static size_t flac_read_flac2d(SF_PRIVATE *psf, double *ptr, size_t len);

static size_t flac_write_s2flac(SF_PRIVATE *psf, const short *ptr, size_t len);
static size_t flac_write_i2flac(SF_PRIVATE *psf, const int *ptr, size_t len);
static size_t flac_write_f2flac(SF_PRIVATE *psf, const float *ptr, size_t len);
static size_t flac_write_d2flac(SF_PRIVATE *psf, const double *ptr, size_t len);
...
static void f2flac24_array(const float *src, int32_t *dest, size_t count,
                           int normalize);
static void f2flac8_clip_array(const float *src, int32_t *dest, size_t count,
                               int normalize);
static void f2flac16_clip_array(const float *src, int32_t *dest, size_t count,
                                int normalize);
static void f2flac24_clip_array(const float *src, int32_t *dest, size_t count,
                                int normalize);
static void d2flac8_array(const double *src, int32_t *dest, size_t count,
                          int normalize);
static void d2flac16_array(const double *src, int32_t *dest, size_t count,
                           int normalize);
static void d2flac24_array(const double *src, int32_t *dest, size_t count,
                           int normalize);
static void d2flac8_clip_array(const double *src, int32_t *dest, size_t count,
                               int normalize);
static void d2flac16_clip_array(const double *src, int32_t *dest, size_t count,
                                int normalize);
static void d2flac24_clip_array(const double *src, int32_t *dest, size_t count,
                                int normalize);

static size_t flac_command(SF_PRIVATE *psf, int command, void *data,
                        size_t datasize);

/* Decoder Callbacks */
static FLAC__StreamDecoderReadStatus
sf_flac_read_callback(const FLAC__StreamDecoder *decoder, FLAC__byte buffer[],
                      size_t *bytes, void *client_data);
static FLAC__StreamDecoderSeekStatus
sf_flac_seek_callback(const FLAC__StreamDecoder *decoder,
                      FLAC__uint64 absolute_byte_offset, void *client_data);
static FLAC__StreamDecoderTellStatus
sf_flac_tell_callback(const FLAC__StreamDecoder *decoder,
                      FLAC__uint64 *absolute_byte_offset, void *client_data);
static FLAC__StreamDecoderLengthStatus
sf_flac_length_callback(const FLAC__StreamDecoder *decoder,
                        FLAC__uint64 *stream_length, void *client_data);
static FLAC__bool sf_flac_eof_callback(const FLAC__StreamDecoder *decoder,
                                       void *client_data);
static FLAC__StreamDecoderWriteStatus
sf_flac_write_callback(const FLAC__StreamDecoder *decoder,
                       const FLAC__Frame *frame, const int32_t *const buffer[],
                       void *client_data);
...
sf_flac_enc_seek_callback(const FLAC__StreamEncoder *encoder,
                          FLAC__uint64 absolute_byte_offset, void *client_data);
static FLAC__StreamEncoderTellStatus
sf_flac_enc_tell_callback(const FLAC__StreamEncoder *encoder,
                          FLAC__uint64 *absolute_byte_offset,
                          void *client_data);
static FLAC__StreamEncoderWriteStatus sf_flac_enc_write_callback(
...
static sf_count_t flac_buffer_copy(SF_PRIVATE *psf)
{
	...
	*/
	...
	if (frame->header.channels > FLAC__MAX_CHANNELS)
		psf_log_printf(
		    psf,
		    "Ooops : frame->header.channels (%d) > FLAC__MAX_BLOCK_SIZE (%d)\n",
	...
	if (pflac->ptr == NULL)
	{
		...
		for (i = 0; i < channels; i++)
		{
			if (pflac->rbuffer[i] == NULL)
				pflac->rbuffer[i] =
				    calloc(FLAC__MAX_BLOCK_SIZE, sizeof(int32_t));
	...
	case PFLAC_PCM_SHORT:
	{
		...
		else
		{
			for (i = 0; i < len && pflac->remain > 0; i++)
			{
				...
				for (j = 0; j < channels; j++)
					retpcm[offset + j] = ((uint16_t)buffer[j][pflac->bufferpos])
					                     << shift;
	...
	case PFLAC_PCM_INT:
	{
		...
		for (i = 0; i < len && pflac->remain > 0; i++)
		{
			...
			for (j = 0; j < channels; j++)
				retpcm[offset + j] = ((uint32_t)buffer[j][pflac->bufferpos])
				                     << shift;
			pflac->remain -= channels;
			pflac->bufferpos++;
		};
	...
	case PFLAC_PCM_DOUBLE:
	{
		...
		double norm = (psf->norm_double == SF_TRUE)
		              ? 1.0 / (1 << (frame->header.bits_per_sample - 1))
		              : 1.0;
...
static FLAC__bool
sf_flac_eof_callback(const FLAC__StreamDecoder *UNUSED(decoder),
                     void *client_data)
{
	SF_PRIVATE *psf = (SF_PRIVATE *)client_data;
...
static FLAC__StreamDecoderWriteStatus
sf_flac_write_callback(const FLAC__StreamDecoder *UNUSED(decoder),
                       const FLAC__Frame *frame, const int32_t *const buffer[],
                       void *client_data)
{
	SF_PRIVATE *psf = (SF_PRIVATE *)client_data;
...
static void
sf_flac_meta_get_vorbiscomments(SF_PRIVATE *psf,
                                const FLAC__StreamMetadata *metadata)
{
	...
	{
		tag_num = FLAC__metadata_object_vorbiscomment_find_entry_from(
		              metadata, 0, tags[k].tag);

		if (tag_num < 0)
			continue;

		value =
		    (const char *)metadata->data.vorbis_comment.comments[tag_num].entry;
                                  ...
                                  void *client_data)
{
	...
	case FLAC__METADATA_TYPE_STREAMINFO:
		...
		{
			psf_log_printf(
			    psf, "Error: FLAC stream changed from %d to %d channels\n"
			    "Nothing to do but to error out.\n",
		...
		case 24:
			...
			break;
		default:
			psf_log_printf(psf, "sf_flac_meta_callback : bits_per_sample %d "
			               "not yet implemented.\n",
...
sf_flac_enc_write_callback(const FLAC__StreamEncoder *UNUSED(encoder),
                           ...
                           unsigned UNUSED(current_frame), void *client_data)
{
	SF_PRIVATE *psf = (SF_PRIVATE *)client_data;

	if (psf_fwrite(buffer, 1, bytes, psf) == (sf_count_t)bytes &&
	        psf->error == 0)
...
static void flac_write_strings(SF_PRIVATE *psf, FLAC_PRIVATE *pflac)
{
	...
	if (pflac->metadata == NULL &&
	        (pflac->metadata = FLAC__metadata_object_new(
	                               FLAC__METADATA_TYPE_VORBIS_COMMENT)) == NULL)
	{
	...
	for (k = 0; k < SF_MAX_STRINGS && psf->strings.data[k].type != 0; k++)
	{
		...
		FLAC__metadata_object_vorbiscomment_append_comment(
		    pflac->metadata, entry, /* copy */ SF_FALSE);
	};
...
static int flac_write_header(SF_PRIVATE *psf, int UNUSED(calc_length))
{
	FLAC_PRIVATE *pflac = (FLAC_PRIVATE *)psf->codec_data;
	int err;
	...
	if ((err = FLAC__stream_encoder_init_stream(
	               ...
	               sf_flac_enc_tell_callback, NULL, psf)) !=
...
static int flac_enc_init(SF_PRIVATE *psf)
{
	...
	{
		psf_log_printf(
		    psf, "FLAC__stream_encoder_set_sample_rate (%d) returned false.\n",
		    psf->sf.samplerate);
		return SFE_FLAC_BAD_SAMPLE_RATE;
	};
	...
	if (!FLAC__stream_encoder_set_compression_level(pflac->fse,
	        pflac->compression))
	{
		psf_log_printf(
		    psf,
		    "FLAC__stream_encoder_set_compression_level (%d) return false.\n",
		    pflac->compression);
		return SFE_FLAC_INIT_DECODER;
	};
...
static int flac_read_header(SF_PRIVATE *psf)
{
	...
	if (FLAC__stream_decoder_init_stream(
	            pflac->fsd, sf_flac_read_callback, sf_flac_seek_callback,
	            sf_flac_tell_callback, sf_flac_length_callback,
	            sf_flac_eof_callback, sf_flac_write_callback, sf_flac_meta_callback,
	            sf_flac_error_callback, psf) != FLAC__STREAM_DECODER_INIT_STATUS_OK)
		return SFE_FLAC_INIT_DECODER;
...
static unsigned flac_read_loop(SF_PRIVATE *psf, size_t len)
{
	FLAC_PRIVATE *pflac = (FLAC_PRIVATE *)psf->codec_data;
	...
	if (pflac->frame != NULL &&
	        pflac->bufferpos < pflac->frame->header.blocksize)
		flac_buffer_copy(psf);
...
static size_t flac_read_flac2s(SF_PRIVATE *psf, short *ptr, size_t len)
{
	...
	while (total < len)
	{
		pflac->ptr = ptr + total;
		readlen =
		    (len - total > 0x1000000) ? 0x1000000 : (len - total);
...
static size_t flac_read_flac2f(SF_PRIVATE *psf, float *ptr, size_t len)
{
	...
	while (total < len)
	{
		pflac->ptr = ptr + total;
		readlen =
		    (len - total > 0x1000000) ? 0x1000000 : (unsigned)(len - total);
		current = flac_read_loop(psf, readlen);
		if (current == 0)
			break;
		total += current;
...
static sf_count_t flac_seek(SF_PRIVATE *psf, int UNUSED(mode),
                            sf_count_t offset)
{
	FLAC_PRIVATE *pflac = (FLAC_PRIVATE *)psf->codec_data;
...
{
	psf_log_printf(
	    psf, "This version of libsndfile was compiled without FLAC support.\n");
	return SFE_UNIMPLEMENTED;
}
