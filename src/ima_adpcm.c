...
static int ima_reader_init(SF_PRIVATE *psf, int blockalign,
                           int samplesperblock);
static int ima_writer_init(SF_PRIVATE *psf, int blockalign);

static size_t ima_read_block(SF_PRIVATE *psf, IMA_ADPCM_PRIVATE *pima,
                             short *ptr, size_t len);
static size_t ima_write_block(SF_PRIVATE *psf, IMA_ADPCM_PRIVATE *pima,
                              const short *ptr, size_t len);
...
static sf_count_t wavlike_ima_seek(SF_PRIVATE *psf, int mode,
                                   sf_count_t offset);
...
static int ima_close(SF_PRIVATE *psf)
{
	...
	if (psf->file.mode == SFM_WRITE)
	{
		...
		psf->sf.frames =
		    pima->samplesperblock * pima->blockcount / psf->sf.channels;
	};
...
static int ima_reader_init(SF_PRIVATE *psf, int blockalign, int samplesperblock)
{
	...
	pima->block =
	    (unsigned char *)(pima->data + samplesperblock * psf->sf.channels);
	...
	{
		psf_log_printf(psf,
		               "*** Error : pima->samplesperblock should be > 0.\n");
		return SFE_INTERNAL;
	};
...
static int aiff_ima_decode_block(SF_PRIVATE *psf, IMA_ADPCM_PRIVATE *pima)
{
	...
	if ((k = psf_fread(pima->block, 1, pima->blocksize * pima->channels,
	                   psf)) != pima->blocksize * pima->channels)
...
static int aiff_ima_encode_block(SF_PRIVATE *psf, IMA_ADPCM_PRIVATE *pima)
{
	...
	for (chan = 0; chan < pima->channels; chan++)
	{
		...
		pima->block[blockindx + 1] =
		    (pima->samples[chan] & 0x80) + (pima->stepindx[chan] & 0x7F);
	...
	for (chan = 0; chan < pima->channels; chan++)
	{
		for (indx = pima->channels;
		        indx < pima->channels * pima->samplesperblock;
	...
	if ((k = psf_fwrite(pima->block, 1, pima->channels * pima->blocksize,
	                    psf)) != pima->channels * pima->blocksize)
...
static int wavlike_ima_decode_block(SF_PRIVATE *psf, IMA_ADPCM_PRIVATE *pima)
{
	...
	if ((k = psf_fread(pima->block, 1, pima->blocksize, psf)) !=
	        pima->blocksize)
		psf_log_printf(psf, "*** Warning : short read (%d != %d).\n", k,
		               pima->blocksize);
...
static int wavlike_ima_encode_block(SF_PRIVATE *psf, IMA_ADPCM_PRIVATE *pima)
{
	...
	if ((k = psf_fwrite(pima->block, 1, pima->blocksize, psf)) !=
	        pima->blocksize)
		psf_log_printf(psf, "*** Warning : short write (%d != %d).\n", k,
		               pima->blocksize);
                             ...
                             short *ptr, size_t len)
{
	...
	while (indx < len)
	{
		...
		memcpy(&(ptr[indx]),
		       &(pima->samples[pima->samplecount * pima->channels]),
		       count * sizeof(short));
		indx += count;
...
static sf_count_t aiff_ima_seek(SF_PRIVATE *psf, int mode, sf_count_t offset)
{
	...
	if (mode == SFM_READ)
	{
		psf_fseek(psf, psf->dataoffset + newblockaiff * pima->blocksize,
		          SEEK_SET);
                              ...
                              const short *ptr, size_t len)
{
	...
	while (indx < len)
	{
		...
		memcpy(&(pima->samples[pima->samplecount * pima->channels]),
		       &(ptr[total]), count * sizeof(short));
		indx += count;
		pima->samplecount += count / pima->channels;
		total = indx;
