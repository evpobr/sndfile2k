...
static size_t pcm_write_s2sc(SF_PRIVATE *psf, const short *ptr,
                             size_t len);
static size_t pcm_write_s2uc(SF_PRIVATE *psf, const short *ptr,
                             size_t len);
static size_t pcm_write_s2bes(SF_PRIVATE *psf, const short *ptr,
                              size_t len);
static size_t pcm_write_s2les(SF_PRIVATE *psf, const short *ptr,
                              size_t len);
static size_t pcm_write_s2bet(SF_PRIVATE *psf, const short *ptr,
                              size_t len);
static size_t pcm_write_s2let(SF_PRIVATE *psf, const short *ptr,
                              size_t len);
static size_t pcm_write_s2bei(SF_PRIVATE *psf, const short *ptr,
                              size_t len);
static size_t pcm_write_s2lei(SF_PRIVATE *psf, const short *ptr,
                              size_t len);

static size_t pcm_write_i2sc(SF_PRIVATE *psf, const int *ptr,
                             size_t len);
static size_t pcm_write_i2uc(SF_PRIVATE *psf, const int *ptr,
                             size_t len);
static size_t pcm_write_i2bes(SF_PRIVATE *psf, const int *ptr,
                              size_t len);
static size_t pcm_write_i2les(SF_PRIVATE *psf, const int *ptr,
                              size_t len);
static size_t pcm_write_i2bet(SF_PRIVATE *psf, const int *ptr,
                              size_t len);
static size_t pcm_write_i2let(SF_PRIVATE *psf, const int *ptr,
                              size_t len);
static size_t pcm_write_i2bei(SF_PRIVATE *psf, const int *ptr,
                              size_t len);
static size_t pcm_write_i2lei(SF_PRIVATE *psf, const int *ptr,
                              size_t len);

static size_t pcm_write_f2sc(SF_PRIVATE *psf, const float *ptr,
                             size_t len);
static size_t pcm_write_f2uc(SF_PRIVATE *psf, const float *ptr,
                             size_t len);
static size_t pcm_write_f2bes(SF_PRIVATE *psf, const float *ptr,
                              size_t len);
static size_t pcm_write_f2les(SF_PRIVATE *psf, const float *ptr,
                              size_t len);
static size_t pcm_write_f2bet(SF_PRIVATE *psf, const float *ptr,
                              size_t len);
static size_t pcm_write_f2let(SF_PRIVATE *psf, const float *ptr,
                              size_t len);
static size_t pcm_write_f2bei(SF_PRIVATE *psf, const float *ptr,
                              size_t len);
static size_t pcm_write_f2lei(SF_PRIVATE *psf, const float *ptr,
                              size_t len);

static size_t pcm_write_d2sc(SF_PRIVATE *psf, const double *ptr,
                             size_t len);
static size_t pcm_write_d2uc(SF_PRIVATE *psf, const double *ptr,
                             size_t len);
static size_t pcm_write_d2bes(SF_PRIVATE *psf, const double *ptr,
                              size_t len);
static size_t pcm_write_d2les(SF_PRIVATE *psf, const double *ptr,
                              size_t len);
static size_t pcm_write_d2bet(SF_PRIVATE *psf, const double *ptr,
                              size_t len);
static size_t pcm_write_d2let(SF_PRIVATE *psf, const double *ptr,
                              size_t len);
static size_t pcm_write_d2bei(SF_PRIVATE *psf, const double *ptr,
                              size_t len);
static size_t pcm_write_d2lei(SF_PRIVATE *psf, const double *ptr,
                              size_t len);
...
int pcm_init(SF_PRIVATE *psf)
{
	...
	if (CPU_IS_BIG_ENDIAN)
		...
		psf->data_endswap =
		    (psf->endian == SF_ENDIAN_LITTLE) ? SF_FALSE : SF_TRUE;
...
static inline void bes2f_array(short *src, size_t count, float *dest,
                               float normfact)
{
	short value;
...
static inline void let2f_array(tribyte *src, size_t count, float *dest,
                               float normfact)
{
	unsigned char *ucptr;
	int value;
...
static inline void bet2f_array(tribyte *src, size_t count, float *dest,
                               float normfact)
{
	unsigned char *ucptr;
	int value;
...
static inline void lei2f_array(int *src, size_t count, float *dest,
                               float normfact)
{
	int value;
...
static inline void bei2f_array(int *src, size_t count, float *dest,
                               float normfact)
{
	int value;
...
static inline void bes2d_array(short *src, size_t count, double *dest,
                               double normfact)
{
	short value;
...
static inline void let2d_array(tribyte *src, size_t count, double *dest,
                               double normfact)
{
	unsigned char *ucptr;
	int value;
...
static inline void bet2d_array(tribyte *src, size_t count, double *dest,
                               double normfact)
{
	unsigned char *ucptr;
	int value;
...
static inline void lei2d_array(int *src, size_t count, double *dest,
                               double normfact)
{
	int value;
...
static inline void bei2d_array(int *src, size_t count, double *dest,
                               double normfact)
{
	int value;
...
static size_t pcm_read_uc2s(SF_PRIVATE *psf, short *ptr, size_t len)
{
	...
	while (len > 0)
	{
		if (len < bufferlen)
			bufferlen = len;
		readcount =
		    psf_fread(ubuf.ucbuf, sizeof(unsigned char), bufferlen, psf);
...
static size_t pcm_read_bet2d(SF_PRIVATE *psf, double *ptr, size_t len)
{
	...
	normfact = (psf->norm_double == SF_TRUE) ? 1.0 / ((double)0x80000000)
	           : 1.0 / 256.0;
                             ...
                             size_t len)
{
	...
	while (len > 0)
	{
		...
		writecount =
		    psf_fwrite(ubuf.scbuf, sizeof(signed char), bufferlen, psf);
		total += writecount;
                             ...
                             size_t len)
{
	...
	while (len > 0)
	{
		...
		writecount =
		    psf_fwrite(ubuf.ucbuf, sizeof(unsigned char), bufferlen, psf);
		total += writecount;
		if (writecount < bufferlen)
			break;
		len -= writecount;
                             ...
                             size_t len)
{
	...
	while (len > 0)
	{
		...
		writecount =
		    psf_fwrite(ubuf.ucbuf, sizeof(signed char), bufferlen, psf);
		total += writecount;
		if (writecount < bufferlen)
			break;
		len -= writecount;
...
static void f2uc_array(const float *src, unsigned char *dest, size_t count,
                       int normalize)
{
	float normfact;
...
static void f2uc_clip_array(const float *src, unsigned char *dest, size_t count,
                            int normalize)
{
	float normfact, scaled_value;
...
static void f2bes_array(const float *src, short *dest, size_t count,
                        int normalize)
{
	unsigned char *ucptr;
	float normfact;
...
static void f2bes_clip_array(const float *src, short *dest, size_t count,
                             int normalize)
{
	unsigned char *ucptr;
	float normfact, scaled_value;
...
static void f2les_array(const float *src, short *dest, size_t count,
                        int normalize)
{
	unsigned char *ucptr;
	float normfact;
...
static void f2les_clip_array(const float *src, short *dest, size_t count,
                             int normalize)
{
	unsigned char *ucptr;
	float normfact, scaled_value;
...
static void f2let_array(const float *src, tribyte *dest, size_t count,
                        int normalize)
{
	unsigned char *ucptr;
	float normfact;
...
static void f2let_clip_array(const float *src, tribyte *dest, size_t count,
                             int normalize)
{
	unsigned char *ucptr;
	float normfact, scaled_value;
...
static size_t pcm_write_f2let(SF_PRIVATE *psf, const float *ptr, size_t len)
{
	...
	while (len > 0)
	{
		if (len < bufferlen)
			bufferlen = len;
		convert(ptr + total, (tribyte *)(ubuf.ucbuf), bufferlen,
		        psf->norm_float);
		writecount = psf_fwrite(ubuf.ucbuf, SIZEOF_TRIBYTE, bufferlen, psf);
		total += writecount;
		if (writecount < bufferlen)
...
static void f2bet_array(const float *src, tribyte *dest, size_t count,
                        int normalize)
{
	unsigned char *ucptr;
	float normfact;
...
static void f2bet_clip_array(const float *src, tribyte *dest, size_t count,
                             int normalize)
{
	unsigned char *ucptr;
	float normfact, scaled_value;
...
static void f2bei_array(const float *src, int *dest, size_t count,
                        int normalize)
{
	unsigned char *ucptr;
	float normfact;
...
static void f2bei_clip_array(const float *src, int *dest, size_t count,
                             int normalize)
{
	unsigned char *ucptr;
	float normfact, scaled_value;
...
static void f2lei_array(const float *src, int *dest, size_t count,
                        int normalize)
{
	unsigned char *ucptr;
	float normfact;
...
static void f2lei_clip_array(const float *src, int *dest, size_t count,
                             int normalize)
{
	unsigned char *ucptr;
	float normfact, scaled_value;
...
static void d2sc_array(const double *src, signed char *dest, size_t count,
                       int normalize)
{
	double normfact;
...
static void d2sc_clip_array(const double *src, signed char *dest, size_t count,
                            int normalize)
{
...
static void d2uc_array(const double *src, unsigned char *dest, size_t count,
                       int normalize)
{
	double normfact;
...
static void d2uc_clip_array(const double *src, unsigned char *dest,
                            size_t count, int normalize)
{
	double normfact, scaled_value;
...
static void d2bes_array(const double *src, short *dest, size_t count,
                        int normalize)
{
	unsigned char *ucptr;
	short value;
...
static void d2bes_clip_array(const double *src, short *dest, size_t count,
                             int normalize)
{
	unsigned char *ucptr;
	double normfact, scaled_value;
...
static void d2les_array(const double *src, short *dest, size_t count,
                        int normalize)
{
	unsigned char *ucptr;
	short value;
...
static void d2les_clip_array(const double *src, short *dest, size_t count,
                             int normalize)
{
	unsigned char *ucptr;
	int value;
...
static void d2let_array(const double *src, tribyte *dest, size_t count,
                        int normalize)
{
	unsigned char *ucptr;
	int value;
...
static void d2let_clip_array(const double *src, tribyte *dest, size_t count,
                             int normalize)
{
	unsigned char *ucptr;
	int value;
...
static void d2bet_array(const double *src, tribyte *dest, size_t count,
                        int normalize)
{
	unsigned char *ucptr;
	int value;
...
static void d2bet_clip_array(const double *src, tribyte *dest, size_t count,
                             int normalize)
{
	unsigned char *ucptr;
	int value;
...
static void d2bei_array(const double *src, int *dest, size_t count,
                        int normalize)
{
	unsigned char *ucptr;
	int value;
...
static void d2bei_clip_array(const double *src, int *dest, size_t count,
                             int normalize)
{
	unsigned char *ucptr;
	int value;
...
static void d2lei_array(const double *src, int *dest, size_t count,
                        int normalize)
{
	unsigned char *ucptr;
	int value;
...
static void d2lei_clip_array(const double *src, int *dest, size_t count,
                             int normalize)
{
	unsigned char *ucptr;
	int value;
