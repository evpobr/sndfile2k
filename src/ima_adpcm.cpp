...
static int ima_close(SF_PRIVATE *psf)
{
    ...
    pima = (IMA_ADPCM_PRIVATE *)psf->m_codec_data;
...
static int ima_reader_init(SF_PRIVATE *psf, int blockalign, int samplesperblock)
{
    ...
    if (psf->m_datalength % pima->blocksize)
        pima->blocks = psf->m_datalength / pima->blocksize + 1;
    else
        pima->blocks = psf->m_datalength / pima->blocksize;
...
static sf_count_t aiff_ima_seek(SF_PRIVATE *psf, int mode, sf_count_t offset)
{
    ...
    if (mode == SFM_READ)
    {
        psf->fseek(psf->m_dataoffset + newblockaiff * pima->blocksize, SEEK_SET);
...
static sf_count_t wavlike_ima_seek(SF_PRIVATE *psf, int mode, sf_count_t offset)
{
    ...
    if (mode == SFM_READ)
    {
        psf->fseek(psf->m_dataoffset + newblock * pima->blocksize, SEEK_SET);
