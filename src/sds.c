...
#define SDS_INT_TO_3BYTE_ENCODE(x) \
...
static size_t sds_read(SF_PRIVATE *psf, SDS_PRIVATE *psds, int *iptr,
                       size_t readcount);
...
static size_t sds_write(SF_PRIVATE *psf, SDS_PRIVATE *psds, const int *iptr,
                        size_t writecount);
	...
	if (psf->file.mode == SFM_WRITE || psf->file.mode == SFM_RDWR)
	{
		...
		if (psds->write_count > 0)
		{
			memset(&(psds->write_data[psds->write_count]), 0,
			       (psds->samplesperblock - psds->write_count) * sizeof(int));
...
static int sds_read_header(SF_PRIVATE *psf, SDS_PRIVATE *psds)
{
	...
	bytesread +=
	    psf_binheader_readf(psf, "e3331", &data_length, &sustain_loop_start,
	...
	psds->samplesperblock =
	    SDS_AUDIO_BYTES_PER_BLOCK / ((psds->bitwidth + 6) / 7);
...
static int sds_write_header(SF_PRIVATE *psf, int calc_length)
{
	...
	psf_binheader_writef(psf, "e213", BHW2(0), BHW1(psds->bitwidth),
	                     BHW3(samp_period));
	...
	psf_binheader_writef(psf, "e33311", BHW3(data_length),
	                     BHW3(sustain_loop_start), BHW3(sustain_loop_end),
	                     BHW1(loop_type), BHW1(0xF7));
...
static int sds_2byte_read(SF_PRIVATE *psf, SDS_PRIVATE *psds)
{
	...
	{
		sample =
		    arith_shift_left(ucptr[k], 25) + arith_shift_left(ucptr[k + 1], 18);
...
static int sds_3byte_read(SF_PRIVATE *psf, SDS_PRIVATE *psds)
{
	...
	for (k = 0; k < 120; k += 3)
	{
		sample = (((uint32_t)ucptr[k]) << 25) + (ucptr[k + 1] << 18) +
		         (ucptr[k + 2] << 11);
...
static sf_count_t sds_seek(SF_PRIVATE *psf, int mode,
...
static int sds_2byte_write(SF_PRIVATE *psf, SDS_PRIVATE *psds)
{
	...
	if ((k = psf_fwrite(psds->write_data, 1, SDS_BLOCK_SIZE, psf)) !=
	        SDS_BLOCK_SIZE)
...
static size_t sds_write_d(SF_PRIVATE *psf, const double *ptr,
                          size_t len)
{
	BUF_UNION ubuf;
	SDS_PRIVATE *psds;
...
static size_t sds_write(SF_PRIVATE *psf, SDS_PRIVATE *psds, const int *ptr,
                        size_t len)
{
	size_t count, total = 0;
