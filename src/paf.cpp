...
static int paf_read_header(SF_PRIVATE *psf)
{
    ...
    case PAF_PCM_24:
        ...
        psf->blockwidth = 0;
        psf->sf.frames =
            PAF24_SAMPLES_PER_BLOCK * psf->datalength / (PAF24_BLOCK_SIZE * psf->sf.channels);
        break;
...
static size_t paf24_read_f(SF_PRIVATE *psf, float *ptr, size_t len)
{
    ...
    normfact = (float)((psf->norm_float == SF_TRUE) ? (1.0 / 0x80000000) : (1.0 / 0x100));
...
static size_t paf24_write_f(SF_PRIVATE *psf, const float *ptr, size_t len)
{
    ...
    normfact = (float)((psf->norm_float == SF_TRUE) ? (1.0 * 0x7FFFFFFF) : (1.0 / 0x100));
