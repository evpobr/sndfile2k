...
DEFUN_DLD(sfversion, args, nargout, "-*- texinfo -*-\n\
...
DEFUN_DLD(sfread, args, nargout, "-*- texinfo -*-\n\
...
@end deftypefn")
{
    ...
    {
        error("sfread: couldn't open file %s : %s", filename.c_str(),
              sf_strerror(NULL));
        return retval;
    };
...
")
{
    ...
    if (nargin != 4 || !args(0).is_string() || !args(1).is_real_matrix() ||
        !args(2).is_real_scalar() || !args(3).is_string() || nargout != 0)
    {
        print_usage();
        return retval;
    };
    ...
    {
        error(
            "Audio data should have one column per channel, but supplied data "
    ...
    {
        error("Couldn't open file %s : %s", filename.c_str(),
              sf_strerror(NULL));
        return retval;
    };
...
static void str_split(const std::string &str, const std::string &delim,
...
static int hash_of_str(const std::string &str)
{
    int hash = 0;
...
static int major_format_of_hash(const std::string &str)
{
    int hash;
...
static int minor_format_of_hash(const std::string &str)
{
    int hash;
...
static const char *string_of_major_format(int format)
{
...
static const char *string_of_minor_format(int format)
{
...
static void string_of_format(std::string &fmt, int format)
{
    ...
    snprintf(buffer, sizeof(buffer), "%s-%s", string_of_major_format(format),
