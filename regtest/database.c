...
static int count_callback(REGTEST_DB *db, int argc, char **argv,
                          char **colname);
static int ekey_max_callback(REGTEST_DB *db, int argc, char **argv,
                             char **colname);
...
int db_file_exists(REG_DB *db_handle, const char *filename)
{
    ...
    snprintf(db->cmdbuf, sizeof(db->cmdbuf),
             "select fname from sndfile where fname='%s'", filename);

    db->count = 0;
    err = sqlite3_exec(db->sql, db->cmdbuf, (sqlite3_callback)count_callback,
                       db, &errmsg);
    if (err == 0 && db->count == 1)
        return 1;
...
int db_add_file(REG_DB *db_handle, const char *filepath)
{
    REGTEST_DB *db;
    SNDFILE *sndfile;
    SF_INFO info;
    ...
    snprintf(db->cmdbuf, sizeof(db->cmdbuf),
             "insert into sndfile "
             "(fname, fpath, srate, frames, channels, format, checksum, "
             "logbuf) values"
             "('%s','%s',%d,'%ld', %d, '0x%08x', '0x%08x', '%s');",
             db->filename, db->pathname, info.samplerate, (long)info.frames,
             info.channels, info.format, checksum, db->logbuf);
...
int db_check_file(REG_DB *db_handle, const char *filepath)
{
    REGTEST_DB *db;
    int ekey;
...
int db_check_all(REG_DB *db_handle)
{
    ...
    err = sqlite3_exec(db->sql, db->cmdbuf, (sqlite3_callback)ekey_max_callback,
                       db, &errmsg);
    if (err != SQLITE_OK)
    {
        printf("Line %d : SQL error: %s\n", __LINE__, errmsg);
...
static int get_ekey_from_filename(REGTEST_DB *db, const char *filepath)
{
    ...
    snprintf(db->cmdbuf, sizeof(db->cmdbuf),
             "select ekey from sndfile where fname='%s'", db->filename);

    err =
        sqlite3_get_table(db->sql, db->cmdbuf, &result, &rows, &cols, &errmsg);
    if (err != SQLITE_OK)
    {
        printf("Line %d : SQL error: %s\n", __LINE__, errmsg);
        puts(db->cmdbuf);
...
static int check_file_by_ekey(REGTEST_DB *db, int ekey)
{
    ...
    snprintf(db->cmdbuf, sizeof(db->cmdbuf),
             ...
             "checksum,logbuf from sndfile where ekey='%d'",
             ekey);
...
static void get_filename_pathname(REGTEST_DB *db, const char *filepath)
{
    ...
    if (filepath[0] != '/')
    {
        ...
        db->pathname[slen++] = '/';
        snprintf(db->pathname + slen, sizeof(db->pathname) - slen, "%s",
                 filepath);
    }
    else
