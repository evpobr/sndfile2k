...
static void ceeplusplus_wchar_test(void)
{
#if 0
	...
	{
		SndfileHandle file(filename, SFM_WRITE, SF_FORMAT_WAV | SF_FORMAT_PCM_16, 2,
		                   44100);

		if (file.refCount() != 1)
		{
			printf("\n\n%s %d : Error : Reference count (%d) should be 1.\n\n", __func__,
			       __LINE__, file.refCount());
			exit(1);
		};
...
static void create_file(const char *filename, int format)
{
    SndfileHandle file;
...
static void check_title(const SndfileHandle &file, const char *filename)
{
...
static void read_file(const char *filename, int format)
{
    SndfileHandle file;
    sf_count_t count;
...
static void ceeplusplus_test(const char *filename, int format)
{
...
static void ceeplusplus_extra_test(void)
{
    SndfileHandle file;
...
static void ceeplusplus_rawhandle_test(const char *filename)
{
    SNDFILE *handle;
    {
...
static void ceeplusplus_takeOwnership_test(const char *filename)
{
    SNDFILE *handle;
    {
        SndfileHandle file(filename);
...
static void ceeplusplus_handle_test(const char *filename, int format)
{
