...
static void lcomp_test_int(const char *str, const char *filename, int filetype,
                           double margin);
...
int main(void)
{
    ...
    lcomp_test_int("au_g721", filename,
                   SF_ENDIAN_BIG | SF_FORMAT_AU | SF_FORMAT_G721_32, 0.06);
	...
	*/
    if ((sfinfo.format & SF_FORMAT_SUBMASK) != SF_FORMAT_MS_ADPCM)
        for (k = 0; k < sfinfo.frames - datalen; k++)
            ...
            {
                printf("Line %d : Incorrect sample B (#%d : abs (%d) should be "
                       "< %d).\n",
    ...
    for (m = 0; m < 3; m++)
    {
        ...
        for (k = 0; k < 11; k++)
            if (error_function(data[k] / scale, orig[k + m * 11] / scale,
                               margin))
            {
    ...
    {
        printf("Line %d: sf_seek (SEEK_SET) followed by sf_readf_int failed "
               "(%d, %d).\n",
    ...
    {
        printf("Line %d: sf_seek (SEEK_CUR) with 0 offset failed (%d should be "
               "%" PRId64 ")\n",
    ...
    if (error_function((double)data[0], (double)orig[seekpos], margin) ||
        k != seekpos)
    {
        printf("Line %d: sf_seek (forwards, SEEK_CUR) followed by sf_readf_int "
               "failed (%d, %d) (%d, %" PRId64 ").\n",
    ...
    if (error_function((double)data[0], (double)orig[seekpos], margin) ||
        k != seekpos)
    {
        printf("sf_seek (backwards, SEEK_CUR) followed by sf_readf_int failed "
               "(%d, %d) (%d, %" PRId64 ").\n",
    ...
    {
        printf("Line %d: Return value from sf_readf_int past end of file "
               "incorrect (%d).\n",
               __LINE__, k);
        exit(1);
    };
    ...
    {
        printf("Line %d: sf_seek (SEEK_END) followed by sf_readf_short failed "
               "(%d should be %d).\n",
