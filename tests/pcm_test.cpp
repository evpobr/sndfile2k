...
static void pcm_test_float(const char *filename, int filetype, uint64_t hash, int replace_float)
{
    ...
    if (sfinfo.format != filetype)
    {
        printf("\n\nError (%s:%d) Mono : Returned format incorrect (0x%08X => "
               "0x%08X).\n",
    ...
    if (sfinfo.frames != items)
    {
        printf("\n\nError (%s:%d) Mono : Incorrect number of frames in file. "
               "(%d => %" PRId64 ")\n",
    ...
    if (sfinfo.format != filetype)
    {
        printf("\n\nError (%s:%d) Stereo : Returned format incorrect (0x%08X "
               "=> 0x%08X).\n",
               __FILE__, __LINE__, filetype, sfinfo.format);
        exit(1);
    };
    ...
    {
        printf("\n\nError (%s:%d) Stereo : Incorrect number of frames in file. "
               "(%d => %" PRId64 ")\n",
    ...
    {
        printf("\n\nError (%s:%d) Stereo : Incorrect number of channels in "
               "file.\n",
               __FILE__, __LINE__);
        exit(1);
    };
    ...
    for (sign = -1, k = 0; k < items; k++)
    {
        ...
        if (fabs(data[k]) > 1e-100 && fabs(error / data[k]) > 1e-5)
        {
            printf("\n\nError (%s:%d) Stereo : Incorrect sample (#%d : %f => "
                   "%f).\n",
