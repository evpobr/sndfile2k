...
int main(int argc, char *argv[])
{
    ...
    if (do_all || !strcmp(argv[1], "wav"))
    {
        ...
        string_short_rdwr_test("short_rdwr.rifx",
                               SF_ENDIAN_BIG | SF_FORMAT_WAV);
...
static const char software[] = "software (libsndfile-X.Y.Z)";
static const char artist[] = "The Artist";
static const char copyright[] = "Copyright (c) 2001 Artist";
static const char comment[] = "Comment goes here!!!";
static const char date[] = "2001/01/27";
static const char album[] = "The Album";
static const char license[] = "The license";
static const char title[] = "This is the title";
static const char long_title[] =
    "This is a very long and very boring title for this file";
static const char long_artist[] = "The artist who kept on changing its name";
static const char genre[] = "The genre";
static const char trackno[] = "Track three";
...
static void string_multi_set_test(const char *filename, int typemajor)
{
    static const char new_software[] = "new software (libsndfile-X.Y.Z)",
...
static void software_string_test(const char *filename)
{
    ...
    for (k = 0; k < 50; k++)
    {
        ...
        file = test_open_file_or_die(filename, SFM_WRITE, &info, SF_TRUE,
                                     __LINE__);

        snprintf(
            sfname, MIN(k, sizeof(sfname)), "%s",
            "abcdefghijklmnopqrestvwxyz0123456789abcdefghijklmnopqrestvwxyz");

        exit_if_true(sf_set_string(file, SF_STR_SOFTWARE, sfname),
                     "\n\nLine %d : sf_set_string (f, SF_STR_SOFTWARE, '%s') "
                     "failed : %s\n",
        ...
        file =
            test_open_file_or_die(filename, SFM_READ, &info, SF_TRUE, __LINE__);
        result = sf_get_string(file, SF_STR_SOFTWARE);

        exit_if_true(result == NULL, "\n\nLine %d : sf_get_string (file, "
                                     "SF_STR_SOFTWARE) returned NULL.\n\n",
                     __LINE__);
...
static void string_header_update(const char *filename, int typemajor)
{
    ...
    sf_count_t frames;
    const char *str;
    const int GROW_BUFFER_AMOUNT =
        4; /* this should be less than half the size of the string header */
    ...
    file1 =
        test_open_file_or_die(filename, SFM_READ, &sfinfo1, SF_TRUE, __LINE__);
