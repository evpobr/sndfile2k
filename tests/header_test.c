...
static void update_seek_short_test(const char *filename, int filetype)
{
	...
	*/
    ...
    {
        printf("\n\nError : sf_command (SFC_SET_UPDATE_HEADER_AUTO) return "
               "error : %s\n\n",
               sf_strerror(outfile));
        exit(1);
    };
...
static void extra_header_test(const char *filename, int filetype)
{
    ...
    /* Test the file with extra header data. */
    outfile = test_open_file_or_die(filename, SFM_WRITE, &sfinfo, SF_TRUE, 503);
    sf_set_string(outfile, SF_STR_TITLE, filename);
    test_writef_short_or_die(outfile, k, buffer, frames, 505);
	...
	*/
    ...
    /* Open again for read/write. */
    outfile = test_open_file_or_die(filename, SFM_RDWR, &sfinfo, 532);
	...
	*/
    ...
    for (k = 1; k < 6; k++)
    {
        ...
        if (0)
        {
            infile = test_open_file_or_die(filename, SFM_READ, &sfinfo, 558);
            check_log_buffer_or_die(infile, 559);
            sf_close(infile);
        };
