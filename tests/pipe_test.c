...
static FILETYPE read_write_types[] = {{SF_FORMAT_RAW, "raw"},
                                      {SF_FORMAT_AU, "au"},
                                      /* Lite remove start */
                                      {SF_FORMAT_PAF, "paf"},
...
static FILETYPE read_only_types[] = {{SF_FORMAT_RAW, "raw"},
                                     {SF_FORMAT_AU, "au"},
                                     {SF_FORMAT_AIFF, "aiff"},
                                     {SF_FORMAT_WAV, "wav"},
                                     {SF_FORMAT_W64, "w64"},
...
static void pipe_read_test(int filetype, const char *ext)
{
    ...
    if ((retval = system(buffer)) != 0)
    {
        ...
        printf("\n\n    Line %d : pipe test returned error for file type "
               "\"%s\".\n\n",
               __LINE__, ext);
        exit(retval);
    };
                                ...
                                SF_INFO *psfinfo_read)
{
    ...
    {
        printf("\n\n%s %d : unable to open unseekable pipe for write type "
               "\"%s\".\n\n",
               __func__, __LINE__, ext);
        exit(1);
    };
    ...
    {
        printf("\n\n%s %d : unable to open unseekable pipe for read type "
               "\"%s\".\n\n",
               __func__, __LINE__, ext);
        exit(1);
    };
...
static void pipe_test_others(FILETYPE *list1, FILETYPE *list2)
{
    SF_FORMAT_INFO info;
    ...
    for (k = 0; k < major_count; k++)
    {
        ...
        if (1)
        {
            ...
            {
                retval = WEXITSTATUS(retval);
                printf("\n\n     Line %d : pipe test should have returned "
                       "error file type \"%s\" but didn't.\n\n",
                       __LINE__, info.name);
                exit(1);
            };
