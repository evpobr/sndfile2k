...
int main(int argc, char *argv[])
{
    ...
    if (do_all || strcmp(argv[1], "rawend") == 0)
    {
        ...
        raw_needs_endswap_test("raw_end.aiff_le",
                               SF_ENDIAN_LITTLE | SF_FORMAT_AIFF);
        test_count++;
    };
...
static void float_norm_test(const char *filename)
{
    ...
    if ((k = sf_write_float(file, float_data, BUFFER_LEN / 2)) !=
        BUFFER_LEN / 2)
    {
...
static void double_norm_test(const char *filename)
{
    ...
    if ((k = sf_write_double(file, double_data, BUFFER_LEN / 2)) !=
        BUFFER_LEN / 2)
    {
...
static void instrumet_rw_test(const char *filename)
{
    ...
    if (sf_command(sndfile, SFC_GET_INSTRUMENT, &inst, sizeof(inst)) == SF_TRUE)
    {
        ...
        if (sf_command(sndfile, SFC_SET_INSTRUMENT, &inst, sizeof(inst)) ==
            SF_TRUE)
...
static void instrument_test(const char *filename, int filetype)
{
    ...
    file =
        test_open_file_or_die(filename, SFM_WRITE, &sfinfo, SF_TRUE, __LINE__);
    if (sf_command(file, SFC_SET_INSTRUMENT, &write_inst, sizeof(write_inst)) ==
        SF_FALSE)
    {
    ...
    file =
        test_open_file_or_die(filename, SFM_READ, &sfinfo, SF_TRUE, __LINE__);
    if (sf_command(file, SFC_GET_INSTRUMENT, &read_inst, sizeof(read_inst)) ==
        SF_FALSE)
    {
    ...
    if (memcmp(&write_inst, &read_inst, sizeof(write_inst)) != 0)
    {
        ...
        printf("W  Base Note : %u\n"
               ...
               "   mode      : %d\n"
               "   start     : %d\tend       : %d\tcount  :%d\n\n",
               write_inst.basenote, write_inst.detune, write_inst.key_lo,
               write_inst.key_hi, write_inst.velocity_lo,
               write_inst.velocity_hi, write_inst.gain, write_inst.loop_count,
        ...
        printf("R  Base Note : %u\n"
               ...
               "   mode      : %d\n"
               "   start     : %d\tend       : %d\tcount  :%d\n\n",
               read_inst.basenote, read_inst.detune, read_inst.key_lo,
               read_inst.key_hi, read_inst.velocity_lo, read_inst.velocity_hi,
               read_inst.gain, read_inst.loop_count, read_inst.loops[0].mode,
               read_inst.loops[0].start, read_inst.loops[0].end,
               read_inst.loops[0].count, read_inst.loops[1].mode,
               read_inst.loops[1].start, read_inst.loops[1].end,
               read_inst.loops[1].count);
...
static void cue_rw_test(const char *filename)
{
    SNDFILE *sndfile;
    SF_INFO sfinfo;
    SF_CUES cues;
    ...
    exit_if_true(sf_command(sndfile, SFC_GET_CUE_COUNT, &cues.cue_count,
                            sizeof(cues.cue_count)) != SF_TRUE,
                 "\nLine %d: SFC_GET_CUE_COUNT command failed.\n\n", __LINE__);

    exit_if_true(cues.cue_count != 3,
...
static void cue_test(const char *filename, int filetype)
{
    ...
    file =
        test_open_file_or_die(filename, SFM_WRITE, &sfinfo, SF_TRUE, __LINE__);
    if (sf_command(file, SFC_SET_CUE, &write_cue, sizeof(write_cue)) ==
        SF_FALSE)
    {
    ...
    if (memcmp(&write_cue, &read_cue, sizeof(write_cue)) != 0)
    {
        ...
        printf(
            ...
            "   sample_offset : %u\n"
            "   name           : %s\n",
            write_cue.cue_count, write_cue.cue_points[0].indx,
            write_cue.cue_points[0].position, write_cue.cue_points[0].fcc_chunk,
            write_cue.cue_points[0].chunk_start,
            write_cue.cue_points[0].block_start,
            write_cue.cue_points[0].sample_offset, write_cue.cue_points[0].name,
            write_cue.cue_points[1].indx, write_cue.cue_points[1].position,
        ...
        printf(
            ...
            "   sample_offset : %u\n"
            "   name          : %s\n",
            read_cue.cue_count, read_cue.cue_points[0].indx,
            read_cue.cue_points[0].position, read_cue.cue_points[0].fcc_chunk,
            read_cue.cue_points[0].chunk_start,
            read_cue.cue_points[0].block_start,
            read_cue.cue_points[0].sample_offset, read_cue.cue_points[0].name,
            read_cue.cue_points[1].indx, read_cue.cue_points[1].position,
...
static void current_sf_info_test(const char *filename)
{
    ...
    outfile =
        test_open_file_or_die(filename, SFM_WRITE, &outinfo, SF_TRUE, __LINE__);
    ...
    exit_if_true(outinfo.frames != BUFFER_LEN,
                 "\n\nLine %d : Initial sfinfo.frames (%" PRId64
                 ") should be %d.\n\n",
                 __LINE__, outinfo.frames, BUFFER_LEN);

    /* Read file making sure no channel map exists. */
    memset(&ininfo, 0, sizeof(ininfo));
    infile =
        test_open_file_or_die(filename, SFM_READ, &ininfo, SF_TRUE, __LINE__);
    ...
    exit_if_true(ininfo.frames != BUFFER_LEN,
                 "\n\nLine %d : Initial sfinfo.frames (%" PRId64
                 ") should be %d.\n\n",
                 __LINE__, ininfo.frames, BUFFER_LEN);
...
static void broadcast_test(const char *filename, int filetype)
{
    ...
    snprintf(bc_write.description, sizeof(bc_write.description),
             "Test description");
    snprintf(bc_write.originator, sizeof(bc_write.originator),
             "Test originator");
    snprintf(bc_write.originator_reference,
             sizeof(bc_write.originator_reference), "%08x-%08x",
             (unsigned int)time(NULL), (unsigned int)(~time(NULL)));
    snprintf(bc_write.origination_date, sizeof(bc_write.origination_date),
             "%d/%02d/%02d", 2006, 3, 30);
    snprintf(bc_write.origination_time, sizeof(bc_write.origination_time),
             "%02d:%02d:%02d", 20, 27, 0);
    ...
    file =
        test_open_file_or_die(filename, SFM_READ, &sfinfo, SF_TRUE, __LINE__);
    if (sf_command(file, SFC_GET_BROADCAST_INFO, &bc_read, sizeof(bc_read)) ==
        SF_FALSE)
    {
    ...
    if (memcmp(bc_write.description, bc_read.description,
               sizeof(bc_write.description)) != 0)
    {
    ...
    if (memcmp(bc_write.originator, bc_read.originator,
               sizeof(bc_write.originator)) != 0)
    {
    ...
    if (memcmp(bc_write.originator_reference, bc_read.originator_reference,
               sizeof(bc_write.originator_reference)) != 0)
    {
        printf("\n\nLine %d : originator_reference mismatch :\n\twrite : "
               "'%s'\n\tread  : '%s'\n\n",
               __LINE__, bc_write.originator_reference,
               bc_read.originator_reference);
        errors++;
    };

    if (memcmp(bc_write.origination_date, bc_read.origination_date,
               sizeof(bc_write.origination_date)) != 0)
    {
    ...
    if (memcmp(bc_write.origination_time, bc_read.origination_time,
               sizeof(bc_write.origination_time)) != 0)
    {
...
static void broadcast_rdwr_test(const char *filename, int filetype)
{
    ...
    snprintf(binfo.originator_reference, sizeof(binfo.originator_reference),
             "%08x-%08x", (unsigned int)time(NULL),
             (unsigned int)(~time(NULL)));
    snprintf(binfo.origination_date, sizeof(binfo.origination_date),
             "%d/%02d/%02d", 2006, 3, 30);
    snprintf(binfo.origination_time, sizeof(binfo.origination_time),
             "%02d:%02d:%02d", 20, 27, 0);
    ...
    if (sf_command(file, SFC_SET_BROADCAST_INFO, &binfo, sizeof(binfo)) !=
        SF_FALSE)
    {
        printf("\n\nLine %d : sf_command (SFC_SET_BROADCAST_INFO) should have "
               "failed but didn't.\n\n",
               __LINE__);
        exit(1);
    };
...
static void check_coding_history_newlines(const char *filename)
{
    static SF_BROADCAST_INFO bc_write, bc_read;
    SNDFILE *file;
    SF_INFO sfinfo;
    ...
    snprintf(bc_write.origination_time, sizeof(bc_write.origination_time),
             "%02d:%02d:%02d", 20, 27, 0);
    snprintf(bc_write.umid, sizeof(bc_write.umid), "Some umid");
    bc_write.coding_history_size =
        snprintf(bc_write.coding_history, sizeof(bc_write.coding_history),
    ...
    for (k = 1; k < bc_read.coding_history_size; k++)
    {
        ...
        if (bc_read.coding_history[k] == '\r' &&
            bc_read.coding_history[k + 1] != '\n')
        {
            printf("\n\nLine %d : '\\r' without '\\n' after.\n\n", __LINE__);
            exit(1);
        };
        ...
        if (bc_read.coding_history[k] == 0 &&
            k < bc_read.coding_history_size - 1)
        {
            printf("\n\nLine %d : '\\0' within coding history at index %d of "
                   "%d.\n\n",
...
static void broadcast_coding_history_test(const char *filename)
{
    ...
    if (bc_read.coding_history_size < strlen(default_history) ||
        memcmp(bc_read.coding_history, default_history,
               strlen(default_history)) != 0)
    {
        printf("\n\n"
    ...
    bc_write.coding_history_size =
        snprintf(bc_write.coding_history, sizeof(bc_write.coding_history), "%s",
                 supplied_history);
    ...
    if (strstr(bc_read.coding_history, supplied_history) !=
        bc_read.coding_history)
    {
...
static void broadcast_coding_history_size(const char *filename)
{
    ...
    for (k = 0; bc_write.coding_history_size < 512; k++)
    {
        snprintf(bc_write.coding_history + bc_write.coding_history_size,
                 sizeof(bc_write.coding_history) - bc_write.coding_history_size,
                 "line %4d\n", k);
        bc_write.coding_history_size = strlen(bc_write.coding_history);
    };

    exit_if_true(
        bc_write.coding_history_size < 512,
    ...
    exit_if_true(bc_read.coding_history_size < 512, "\n\nLine %d : unexpected "
                                                    "coding history size %d "
                                                    "(should be > 512).\n\n",
                 __LINE__, bc_read.coding_history_size);

    exit_if_true(strstr(bc_read.coding_history, "sndfile2k") == NULL,
                 "\n\nLine %d : coding history incomplete (should contain "
                 "'libsndfile').\n\n",
                 __LINE__);
...
static void cart_test(const char *filename, int filetype)
{
    ...
    snprintf(ca_write.classification, sizeof(ca_write.classification),
             "Test classification");
    snprintf(ca_write.out_cue, sizeof(ca_write.out_cue), "Test out cue");
    snprintf(ca_write.start_date, sizeof(ca_write.start_date), "%d/%02d/%02d",
             2006, 3, 30);
    snprintf(ca_write.start_time, sizeof(ca_write.start_time), "%02d:%02d:%02d",
             20, 27, 0);
    snprintf(ca_write.end_date, sizeof(ca_write.end_date), "%d/%02d/%02d", 2006,
             3, 30);
    snprintf(ca_write.end_time, sizeof(ca_write.end_time), "%02d:%02d:%02d", 20,
             27, 0);
    snprintf(ca_write.producer_app_id, sizeof(ca_write.producer_app_id),
             "Test producer app id");
    snprintf(ca_write.producer_app_version,
             sizeof(ca_write.producer_app_version),
             "Test producer app version");
    snprintf(ca_write.user_def, sizeof(ca_write.user_def),
             "test user def test test");
    ca_write.level_reference = 42;
    snprintf(ca_write.url, sizeof(ca_write.url),
             "http://www.test.com/test_url");
    snprintf(ca_write.tag_text, sizeof(ca_write.tag_text),
             "tag text test! \r\n"); // must be terminated \r\n to be valid
    ca_write.tag_text_size = strlen(ca_write.tag_text);

    file =
        test_open_file_or_die(filename, SFM_WRITE, &sfinfo, SF_TRUE, __LINE__);
    if (sf_command(file, SFC_SET_CART_INFO, &ca_write, sizeof(ca_write)) ==
        SF_FALSE)
        exit(1);
    ...
    file =
        test_open_file_or_die(filename, SFM_READ, &sfinfo, SF_TRUE, __LINE__);
    if (sf_command(file, SFC_GET_CART_INFO, &ca_read, sizeof(ca_read)) ==
        SF_FALSE)
    {
    ...
    if (memcmp(ca_write.version, ca_read.version, sizeof(ca_write.version)) !=
        0)
    {
    ...
    if (memcmp(ca_write.client_id, ca_read.client_id,
               sizeof(ca_write.client_id)) != 0)
    {
    ...
    if (memcmp(ca_write.category, ca_read.category,
               sizeof(ca_write.category)) != 0)
    {
    ...
    if (memcmp(ca_write.out_cue, ca_read.out_cue, sizeof(ca_write.out_cue)) !=
        0)
    {
    ...
    if (memcmp(ca_write.start_date, ca_read.start_date,
               sizeof(ca_write.start_date)) != 0)
    {
    ...
    if (memcmp(ca_write.start_time, ca_read.start_time,
               sizeof(ca_write.start_time)) != 0)
    {
    ...
    if (memcmp(ca_write.end_date, ca_read.end_date,
               sizeof(ca_write.end_date)) != 0)
    {
    ...
    if (memcmp(ca_write.end_time, ca_read.end_time,
               sizeof(ca_write.end_time)) != 0)
    {
    ...
    if (memcmp(ca_write.producer_app_id, ca_read.producer_app_id,
               sizeof(ca_write.producer_app_id)) != 0)
    {
    ...
    if (memcmp(ca_write.producer_app_version, ca_read.producer_app_version,
               sizeof(ca_write.producer_app_version)) != 0)
    {
        printf("\n\nLine %d : producer_app_version mismatch :\n\twrite : "
               "'%s'\n\tread  : '%s'\n\n",
               __LINE__, ca_write.producer_app_version,
               ca_read.producer_app_version);
        errors++;
    };

    if (memcmp(ca_write.user_def, ca_read.user_def,
               sizeof(ca_write.user_def)) != 0)
    {
    ...
    if (memcmp(ca_write.post_timers, ca_read.post_timers,
               sizeof(ca_write.post_timers)) != 0)
    {
    ...
    if (memcmp(ca_write.tag_text, ca_read.tag_text,
               (size_t)(ca_read.tag_text_size)) != 0)
    {
...
static void cart_rdwr_test(const char *filename, int filetype)
{
    ...
    snprintf(cinfo.classification, sizeof(cinfo.classification),
             "Test classification");
    snprintf(cinfo.out_cue, sizeof(cinfo.out_cue), "Test out cue");
    snprintf(cinfo.start_date, sizeof(cinfo.start_date), "%d/%02d/%02d", 2006,
             3, 30);
    snprintf(cinfo.start_time, sizeof(cinfo.start_time), "%02d:%02d:%02d", 20,
             27, 0);
    snprintf(cinfo.end_date, sizeof(cinfo.end_date), "%d/%02d/%02d", 2006, 3,
             30);
    snprintf(cinfo.end_time, sizeof(cinfo.end_time), "%02d:%02d:%02d", 20, 27,
             0);
    snprintf(cinfo.producer_app_id, sizeof(cinfo.producer_app_id),
             "Test producer app id");
    snprintf(cinfo.producer_app_version, sizeof(cinfo.producer_app_version),
             "Test producer app version");
    ...
    {
        printf("\n\nLine %d : sf_command (SFC_SET_CART_INFO) should have "
               "failed but didn't.\n\n",
               __LINE__);
        exit(1);
    };
...
static void channel_map_test(const char *filename, int filetype)
{
    ...
    int channel_map_read[4],
        channel_map_write[4] = {SF_CHANNEL_MAP_LEFT, SF_CHANNEL_MAP_RIGHT,
                                SF_CHANNEL_MAP_LFE, SF_CHANNEL_MAP_REAR_CENTER};
    ...
    case SF_FORMAT_RF64:
        ...
        file = test_open_file_or_die(filename, SFM_READ, &sfinfo, SF_TRUE,
                                     __LINE__);
        exit_if_true(sf_command(file, SFC_GET_CHANNEL_MAP_INFO,
                                channel_map_read,
                                sizeof(channel_map_read)) == SF_FALSE,
                     "\n\nLine %d : sf_command (SFC_GET_CHANNEL_MAP_INFO) "
    ...
    file =
        test_open_file_or_die(filename, SFM_WRITE, &sfinfo, SF_TRUE, __LINE__);
    exit_if_true(
        sf_command(file, SFC_SET_CHANNEL_MAP_INFO, channel_map_write,
    ...
    file =
        test_open_file_or_die(filename, SFM_READ, &sfinfo, SF_TRUE, __LINE__);
    exit_if_true(
        sf_command(file, SFC_GET_CHANNEL_MAP_INFO, channel_map_read,
                   sizeof(channel_map_read)) != SF_TRUE,
        "\n\nLine %d : sf_command (SFC_GET_CHANNEL_MAP_INFO) failed.\n\n",
        __LINE__);
    check_log_buffer_or_die(file, __LINE__);
    sf_close(file);

    exit_if_true(memcmp(channel_map_read, channel_map_write,
                        sizeof(channel_map_read)) != 0,
                 "\n\nLine %d : Channel map read does not match channel map "
                 "written.\n\n",
                 __LINE__);
...
static void raw_needs_endswap_test(const char *filename, int filetype)
{
    static int subtypes[] = {SF_FORMAT_FLOAT, SF_FORMAT_DOUBLE,
                             SF_FORMAT_PCM_16, SF_FORMAT_PCM_24,
                             SF_FORMAT_PCM_32};
    SNDFILE *file;
    SF_INFO sfinfo;
    unsigned k;
