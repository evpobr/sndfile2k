...
static void show_fstat_error(void)
{
    ...
    printf("1) Re-open file in read/write mode and write another %d bytes at "
           "the end.\n",
           SIGNED_SIZEOF(data));
    ...
    printf("4) According to fstat(), the file length is %ld, ",
...
static void show_stat_fstat_error(void)
{
    ...
    printf("1) Now call stat and fstat on the file and retreive the file "
           "lengths.\n");
    ...
    printf("2) Size returned by stat and fstat is %d and %d, ", stat_size,
           fstat_size);
...
static void write_to_closed_file(void)
{
    ...
    printf(
        "4) Now write more data to the file descriptor which should fail.\n");
    if (write(fd, "b\n", 2) < 0)
        printf("5) Good, write returned an error code as it should have.\n");
    else
    {
        printf("5) Attempting to write to a closed file should have failed but "
               "didn't! *** WRONG ***\n");
