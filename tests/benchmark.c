...
static void calc_short_performance(int format, double read_rate,
                                   double write_rate);
static void calc_int_performance(int format, double read_rate,
                                 double write_rate);
static void calc_float_performance(int format, double read_rate,
                                   double write_rate);
...
int main(int argc, char *argv[])
{
    ...
    sf_command(NULL, SFC_GET_LIB_VERSION, buffer + strlen(buffer),
    ...
    if (argc < 2 || strcmp("--native-only", argv[1]) == 0)
    {
        ...
        calc_short_performance(format_major | SF_FORMAT_PCM_16, stats.read_rate,
                               stats.write_rate);
        calc_int_performance(format_major | SF_FORMAT_PCM_24, stats.read_rate,
                             stats.write_rate);
        calc_int_performance(format_major | SF_FORMAT_PCM_32, stats.read_rate,
                             stats.write_rate);
        calc_float_performance(format_major | SF_FORMAT_PCM_16, stats.read_rate,
                               stats.write_rate);
        calc_float_performance(format_major | SF_FORMAT_PCM_24, stats.read_rate,
                               stats.write_rate);
        calc_float_performance(format_major | SF_FORMAT_PCM_32, stats.read_rate,
                               stats.write_rate);
        calc_float_performance(format_major | SF_FORMAT_FLOAT, stats.read_rate,
                               stats.write_rate);
    };
                                   ...
                                   double write_rate)
{
    ...
    while (clock_time < (CLOCKS_PER_SEC * TEST_DURATION))
    {
        ...
        for (k = 0; k < BLOCK_COUNT; k++)
        {
            if ((retval = sf_write_short(file, short_data, item_count)) !=
                item_count)
            {
                printf("Error : sf_write_short returned %d (should have been "
                       "%d)\n",
                       retval, item_count);
                exit(1);
            };
    ...
    while (clock_time < (CLOCKS_PER_SEC * TEST_DURATION))
    {
        ...
        for (k = 0; k < BLOCK_COUNT; k++)
        {
            if ((retval = sf_read_short(file, short_data, item_count)) !=
                item_count)
            {
                printf("Error : write returned %d (should have been %d)\n",
                       retval, item_count);
                                 ...
                                 double write_rate)
{
    ...
    while (clock_time < (CLOCKS_PER_SEC * TEST_DURATION))
    {
        ...
        for (k = 0; k < BLOCK_COUNT; k++)
        {
            if ((retval = sf_write_int(file, int_data, item_count)) !=
                item_count)
            {
                printf("Error : sf_write_short returned %d (should have been "
                       "%d)\n",
    ...
    while (clock_time < (CLOCKS_PER_SEC * TEST_DURATION))
    {
        ...
        for (k = 0; k < BLOCK_COUNT; k++)
        {
            if ((retval = sf_read_int(file, int_data, item_count)) !=
                item_count)
            {
                printf("Error : write returned %d (should have been %d)\n",
                       retval, item_count);
                                   ...
                                   double write_rate)
{
    ...
    while (clock_time < (CLOCKS_PER_SEC * TEST_DURATION))
    {
        ...
        for (k = 0; k < BLOCK_COUNT; k++)
        {
            if ((retval = sf_write_float(file, float_data, item_count)) !=
                item_count)
            {
                printf("Error : sf_write_short returned %d (should have been "
                       "%d)\n",
    ...
    while (clock_time < (CLOCKS_PER_SEC * TEST_DURATION))
    {
        ...
        for (k = 0; k < BLOCK_COUNT; k++)
        {
            if ((retval = sf_read_float(file, float_data, item_count)) !=
                item_count)
            {
                printf("Error : write returned %d (should have been %d)\n",
                       retval, item_count);
